 ...
 * translators {@link DPTXlatorBoolean}, {@link DPTXlator3BitControlled},
 * {@link DPTXlator8BitUnsigned}, {@link DPTXlator2ByteFloat},
 * {@link DPTXlatorString}. Other translator types are loaded through
...
public class ProcessCommunicatorImpl implements ProcessCommunicator {
	private final class NLListener implements NetworkLinkListener {
		...
		public void indication(FrameEvent e) {
			...
			// we nevertheless notify the listeners about it (we do *not*
			// discard it)
			...
			if (svc == GROUP_READ)
				...
				logger.warn("unsupported APDU service - ignored, service code = 0x" + Integer.toHexString(svc));
		}
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.process.ProcessCommunicator#setResponseTimeout(int)
	 */
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.process.ProcessCommunicator#getResponseTimeout()
	 */
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.process.ProcessCommunicator#removeProcessListener
	 * (tuwien.auto.calimero.process.ProcessListener)
	 */
	...
	public float readFloat(GroupAddress dst)
		...
		final DPTXlator2ByteFloat t = new DPTXlator2ByteFloat(DPTXlator2ByteFloat.DPT_TEMPERATURE_DIFFERENCE);
		extractGroupASDU(apdu, t);
	 ...
	 */
	public void write(GroupAddress dst, float value)
			throws KNXTimeoutException, KNXFormatException, KNXLinkClosedException {
		final DPTXlator2ByteFloat t = new DPTXlator2ByteFloat(DPTXlator2ByteFloat.DPT_TEMPERATURE_DIFFERENCE);
		t.setValue(value);
		write(dst, priority, t);
	 ...
	 * (tuwien.auto.calimero.GroupAddress)
	 */
	public String readString(GroupAddress dst)
			throws KNXTimeoutException, KNXRemoteException, KNXLinkClosedException, KNXFormatException {
	 ...
	 * (tuwien.auto.calimero.GroupAddress, java.lang.String)
	 */
	public void write(GroupAddress dst, String value)
			throws KNXTimeoutException, KNXFormatException, KNXLinkClosedException {
		final DPTXlatorString t = new DPTXlatorString(DPTXlatorString.DPT_STRING_8859_1);
		t.setValue(value);
		write(dst, priority, t);
	...
	private synchronized byte[] readFromGroup(GroupAddress dst, Priority p, int minASDULen, int maxASDULen)
			throws KNXTimeoutException, KNXInvalidResponseException, KNXLinkClosedException {
		if (detached)
			throw new KNXIllegalStateException("process communicator detached");
		try {
			wait = true;
	...
	private byte[] waitForResponse(int minAPDU, int maxAPDU) throws KNXInvalidResponseException, KNXTimeoutException {
				...
				try {
					while (indications.size() > 0) {
						...
						if (d.length >= minAPDU && d.length <= maxAPDU)
							...
							// lets wait
							final String s = "APDU response length " + d.length + " bytes, expected " + minAPDU + " to "
									+ maxAPDU;
	 ...
	 *
	 * @param service
	 *            application layer group service code
	 * @param t
	 *            DPT translator with items to put into ASDU
	 ...
	 * length of the supplied <code>apdu</code> is 2, a compact group APDU
	 * format layout is assumed.<br>
	 * On return of this method, the supplied translator contains the DPT items
	 * from the ASDU.
	 *
	 * @param apdu
	 *            application layer protocol data unit, 2 &lt;= apdu.length
	 * @param t
	 *            the DPT translator to fill with the ASDU
	 */
