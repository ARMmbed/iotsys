 ...
 * <p>
 * It stores knowledge about a datapoint in the KNX network, used for
 * communication within Calimero, the KNX network, and the user.<br>
 * The datapoint is identified through a KNX group address. A name is supplied
 * to allow a more friendly interaction with the user, the selected name does
 * not have to be unique. Information exchanged between datapoints consists of a
 * certain encoding, defined by a datapoint type (DPT). This information
 * exchange is done through messages, which are sent with a {@link Priority}
 * associated with the respective datapoint. Every datapoint object can have its
 * own DPT and priority set by using datapoint methods.<br>
 * Note, only information for how to handle interaction is stored within a
 * datapoint type. Neither datapoint values (states or events), nor any
 * datapoint messages are stored.
 * <p>
...
public abstract class Datapoint {
	...
	// main group address is actually final, but left mutable for easier XML
	// loading
	private GroupAddress main;
	private volatile String name;

	// DPT translation, a not set DPT is identified through mainNo=0 and
	// dptID=null
	...
	/**
	 * Creates a new datapoint with a name and specifies state/command based
	 * semantics.
	 * <p>
	 * 
	 * @param main
	 ...
	 * @param stateBased
	 *            <code>true</code> for state based datapoint,
	 *            <code>false</code> for command based datapoint
	 */
	 ...
	 * in the reader. It reads the start tag and attributes of a datapoint
	 * element, and sets the reader to the next position.
	 * 
	 * @param r
	 *            a XML reader
	 ...
	 * @param r
	 *            a XML reader
	 * @return the created datapoint, either of type {@link StateDP} or
	 *         {@link CommandDP}
	 * @throws KNXMLException
	 *             if the XML element is no datapoint or could not be read
	 *             correctly
	...
	/**
	 * Returns the datapoint main address, a KNX group address identifying this
	 * datapoint.
	 * <p>
	 * 
	 ...
	 * <p>
	 * The datapoint name might be any user defined name, it is only used for
	 * interaction with the user.
	 * 
	 * @param friendlyName
	 *            user friendly name of the datapoint
	 */
	 ...
	 * <p>
	 * 
	 * @return <code>true</code> if datapoint is state based, <code>false</code>
	 *         for command based
	 */
	 ...
	 * <p>
	 * A datapoint type is used with <code>DPTXlator</code>s for value
	 * translation.
	 * 
	 * @param mainNumber
	 ...
	 *            be left 0
	 * @param dptID
	 *            the datapoint type used for translation in a DPT translator
	 */
	...
	/**
	 * Returns the main number of the data type to use for datapoint value
	 * translation.
	 * <p>
	 * If the DPT (see {@link #getDPT()}) assigned to this datapoint
	 * unambiguously identifies the DPT translator, the returned main number
	 * might be left 0 by the user of this datapoint.
	 * 
	...
	/**
	 * Returns the datapoint type ID of a DPT translator to use for datapoint
	 * value translation.
	 * <p>
	 * 
	 ...
	 *            a XML writer
	 * @throws KNXMLException
	 *             on error saving this datapoint
	 */
	public void save(XMLWriter w) throws KNXMLException {
		/*
		 * XML layout: <datapoint stateBased=[true|false] name=string
	...
	public String toString() {
		return main.toString() + " " + name + ", DPT main " + mainNo + " id " + (dptID == null ? "-" : dptID) + ", "
