...
public class RemotePropertyServiceAdapter implements PropertyAdapter {
	...
	private final class NLListener implements NetworkLinkListener {
		...
		public void linkClosed(CloseEvent e) {
			pal.adapterClosed(new CloseEvent(RemotePropertyServiceAdapter.this, e.isUserRequest(), e.getReason()));
		}
	}
	 ...
	 * 
	 * @param link
	 *            KNX network link used for communication with the KNX network
	 * @param remote
	 *            KNX individual address to access its interface objects
	 * @param l
	 *            property adapter listener to get notified about adapter
	 *            events, use <code>null</code> for no listener
	 * @param connOriented
	 *            <code>true</code> to use connection oriented mode for access,
	 *            <code>false</code> to use connectionless mode
	 * @throws KNXLinkClosedException
	 *             if the network link is closed
	 */
	public RemotePropertyServiceAdapter(KNXNetworkLink link, IndividualAddress remote, PropertyAdapterListener l,
			boolean connOriented) throws KNXLinkClosedException {
	...
	/**
	 * Creates a new property adapter for remote property access in
	 * connection-oriented mode with authorization.
	 * 
	 * @param link
	 *            KNX network link used for communication with the KNX network
	 * @param remote
	 ...
	 *            events, use <code>null</code> for no listener
	 * @param authorizeKey
	 *            byte array with authorization key
	 * @throws KNXLinkClosedException
	 *             if the network link is closed
	 * @throws KNXException
	 *             on failure during authorization
	 */
	public RemotePropertyServiceAdapter(KNXNetworkLink link, IndividualAddress remote, PropertyAdapterListener l,
			byte[] authorizeKey) throws KNXException {
	 ...
	 * int)
	 */
	public byte[] getDescription(int objIndex, int pid, int propIndex) throws KNXException {
		return mc.readPropertyDesc(dst, objIndex, pid, propIndex);
	}

	/**
	 * {@inheritDoc} The name for this adapter starts with "remote PS " + remote
	 * KNX individual address, allowing easier distinction of adapter types.
	 */
