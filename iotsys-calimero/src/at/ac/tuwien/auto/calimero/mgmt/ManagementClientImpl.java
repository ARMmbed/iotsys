 ...
 * All management service methods invoked after a detach of the network link are
...
public class ManagementClientImpl implements ManagementClient {
	...
	/**
	 * Creates a new management client attached to the supplied KNX network
	 * link.
	 * <p>
	...
	public void writeAddress(IndividualAddress newAddress) throws KNXTimeoutException, KNXLinkClosedException {
		tl.broadcast(false, Priority.SYSTEM, DataUnitBuilder.createAPDU(IND_ADDR_WRITE, newAddress.toByteArray()));
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see tuwien.auto.calimero.mgmt.ManagementClient#writeAddress (byte[],
	 * tuwien.auto.calimero.IndividualAddress)
	 */
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.mgmt.ManagementClient#writeDomainAddress(byte[])
	 */
	...
	public List<byte[]> readDomainAddress(byte[] domain, IndividualAddress start, int range)
		...
		return makeDOAs(readBroadcast(priority,
				DataUnitBuilder.createAPDU(DOA_SELECTIVE_READ,
						new byte[] { domain[0], domain[1], addr[0], addr[1], (byte) range }),
				DOA_RESPONSE, 2, 2, false));
	}
	...
	public byte[] readDeviceDesc(Destination dst, int descType)
			throws KNXInvalidResponseException, KNXDisconnectException, KNXTimeoutException, KNXLinkClosedException {
		if (descType < 0 || descType > 63)
			throw new KNXIllegalArgumentException("descriptor type out of range [0..63]");
		final byte[] apdu = sendWait2(dst, priority,
				DataUnitBuilder.createCompactAPDU(DEVICE_DESC_READ, new byte[] { (byte) descType }),
				DEVICE_DESC_RESPONSE, 2, 14);
	 ...
	 */
	public void restart(Destination dst) throws KNXTimeoutException, KNXLinkClosedException {
	 ...
	 */
	public byte[] readProperty(Destination dst, int objIndex, int propID, int start, int elements)
			throws KNXTimeoutException, KNXRemoteException, KNXDisconnectException, KNXLinkClosedException {
		if (objIndex < 0 || objIndex > 255 || propID < 0 || propID > 255 || start < 0 || start > 0xFFF || elements < 0
				|| elements > 15)
		...
		final byte[] apdu = sendWait2(dst, priority, DataUnitBuilder.createAPDU(PROPERTY_READ, asdu), PROPERTY_RESPONSE,
				4, 14);
	 ...
	 */
	public void writeProperty(Destination dst, int objIndex, int propID, int start, int elements, byte[] data)
			throws KNXTimeoutException, KNXRemoteException, KNXDisconnectException, KNXLinkClosedException {
		if (objIndex < 0 || objIndex > 255 || propID < 0 || propID > 255 || start < 0 || start > 0xFFF
				|| data.length == 0 || elements < 0 || elements > 15)
			throw new KNXIllegalArgumentException("argument value out of range");
		...
		if (data.length != apdu.length - 6)
			throw new KNXInvalidResponseException(
					"data lengths differ, bytes: " + data.length + " written, " + (apdu.length - 6) + " response");
	 ...
	 */
	public byte[] readPropertyDesc(Destination dst, int objIndex, int propID, int propIndex)
			throws KNXTimeoutException, KNXRemoteException, KNXDisconnectException, KNXLinkClosedException {
		if (objIndex < 0 || objIndex > 255 || propID < 0 || propID > 255 || propIndex < 0 || propIndex > 255)
			throw new KNXIllegalArgumentException("argument value out of range");
		final byte[] send = DataUnitBuilder.createAPDU(PROPERTY_DESC_READ,
				new byte[] { (byte) objIndex, (byte) propID, (byte) (propID == 0 ? propIndex : 0) });
	...
	public int readADC(Destination dst, int channelNr, int repeat)
			throws KNXTimeoutException, KNXDisconnectException, KNXRemoteException, KNXLinkClosedException {
		...
		final byte[] apdu = sendWait(dst, priority,
				DataUnitBuilder.createCompactAPDU(ADC_READ, new byte[] { (byte) channelNr, (byte) repeat }),
				ADC_RESPONSE, 3, 3);
		if (apdu[2] == 0)
	...
	public byte[] readMemory(Destination dst, int startAddr, int bytes)
		...
		final byte[] apdu = sendWait(dst, priority,
				DataUnitBuilder.createCompactAPDU(MEMORY_READ,
	...
	public void writeMemory(Destination dst, int startAddr, byte[] data)
			throws KNXDisconnectException, KNXTimeoutException, KNXRemoteException, KNXLinkClosedException {
	 ...
	 */
	public byte authorize(Destination dst, byte[] key)
			throws KNXDisconnectException, KNXTimeoutException, KNXRemoteException, KNXLinkClosedException {
		if (key.length != 4)
		...
		final byte[] apdu = sendWait(dst, priority, DataUnitBuilder.createAPDU(AUTHORIZE_READ, key), AUTHORIZE_RESPONSE,
				1, 1);
	...
	public void writeKey(Destination dst, int level, byte[] key)
		...
		final byte[] apdu = sendWait(dst, priority,
				DataUnitBuilder.createAPDU(KEY_WRITE, new byte[] { (byte) level, key[0], key[1], key[2], key[3] }),
				KEY_RESPONSE, 1, 1);
		if ((apdu[1] & 0xFF) == 0xFF)
	...
	private synchronized byte[] sendWait(Destination d, Priority p, byte[] apdu, int response, int minASDULen,
			int maxASDULen)
			throws KNXDisconnectException, KNXTimeoutException, KNXInvalidResponseException, KNXLinkClosedException {
		try {
	...
	private synchronized byte[] sendWait2(Destination d, Priority p, byte[] apdu, int response, int minASDULen,
			int maxASDULen)
			throws KNXDisconnectException, KNXTimeoutException, KNXInvalidResponseException, KNXLinkClosedException {
		try {
			svcResponse = response;
