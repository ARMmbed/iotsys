 ...
 * <p>
 * This can be done in different ways, to specify the kind of access a property
 * adapter is supplied on creation of the property client. The implementation of
 * the {@link PropertyAdapter} interface methods don't need to be synchronized
 * for use by this property client.
 * <p>
 * Properties can be retrieved or set in the device, property descriptions can
 * be read and scans of properties can be done.<br>
 ...
 * The DPT translators used are requested from {@link PropertyTypes} by default,
 * or, if property definitions were loaded, there will be a lookup in these data
 * at first (i.e. loaded property definitions take priority over the default PDT
 * to DPT mapping).
 * <p>
 * It is possible to load property definitions with information about KNX
 * properties from a resource to be used by the property client for lookup
 * requests and property type translation. Also, definitions can be saved to a
 * resource. A global resource handler takes care of working with the resource
 * where these definitions are stored. Loaded definitions are shared with all
 * property clients.<br>
 * Loading of such definitions is not required for a client to work, i.e. might
 * be done optionally by the user.
 * <p>
 ...
 * &lt;/propertyDefinitions&gt;<br>
 * <br>
 * Attribute values of type number might be written in hexadecimal form by
 * prepending "0x". The optional attribute "dpt" is to specify a DPT to use for
 * the property, it will be used in preference before the default DPT assigned
 * to the PDT.<br>
 * The attribute "pdt" might have a value of "&lt;tbd&gt;", standing for "to be
 * defined", in this case the PDT value used by the property client is -1.
 * <p>
 * Reduced Property Interfaces:<br>
 * When working with reduced property interfaces, the user has to be aware of
 * the limitations and act accordingly.
 * <p>
 * Some KNX devices only support a 5 Bit field for storing the Property Data
 * Type (PDT), i.e. they only use a PDT identifier up to value 0x1F. When
 * accessing property descriptions in the interface object (for example, using
 * {@link PropertyClient#getDescription(int, int)}), not all existing PDTs can
 * be transmitted. Consequently, for properties that are formatted according any
 * of the higher PDTs, "alternative PDTs" get used. Following implementations
 * are known to support only a 5 Bit PDT in the description:
 * <ul>
 * <li>mask 0x0020</li>
 * <li>mask 0x0021</li>
 * <li>mask 0x0701</li>
 * </ul>
 * In general, it is not possible for the property client to deduce the actual
 * type to use for encoding/decoding values from such an "alternative PDT". For
 * these cases, the property client has to rely on property definitions supplied
 * by the user through {@link PropertyClient#loadDefinitions(String)}.
 * <p>
 * A note on property descriptions:<br>
 * With a local device management adapter, not all information is supported when
 * reading a description, or is not supported by the protocol at all. In
 * particular, no access levels for read/write (i.e. access is always done with
 * maximum rights) and no property data type (PDT) are available. Also, the
 * maximum number of elements allowed in the property is not available (only the
 * current number of elements).<br>
 * All methods for property access invoked after a close of the property client
 * will throw a {@link KNXIllegalStateException}.
 * 
 * @author B. Malinowsky
 * @see PropertyAdapter
 * @see PropertyTypes
...
public class PropertyClient {
	/**
	 * Contains constants definitions of the property IDs for global properties
	 * (server object type = global) and PIDs of the KNXnet/IP parameter object
	 * (server object type 11).
	 * <p>
	 * It is encouraged for users to use these more descriptive name constants
	 * instead of plain PID integer values for better readability.<br>
	 * The PIDs are put into its own interface to create an encapsulation
	 * besides the property client interface, since these constants are solely
	 * for the user's sake, the property client itself does not depend on them.
	 * <br>
	 * For this reason, the "interface is type" idiom is not strictly followed
	 * here.
	 * 
	 * @author B. Malinowsky
	 */
	public static interface PID {
		 ...
		 * <p>
		 * Identification of the project and the installation in which this
		 * KNXnet/IP device resides.
		 */
		// PDT,DPT: PDT_UNSIGNED_INT
		 ...
		 * <p>
		 * Sorted list of additional KNX Individual Addresses for KNXnet/IP
		 * routers and tunneling servers.
		 */
		// PDT,DPT: PDT_UNSIGNED_INT[]
		 ...
		 * <p>
		 * IP address of the DHCP or BootP server from which the KNXnet/IP
		 * device received its IP address.
		 */
		// PDT,DPT: PDT_UNSIGNED_LONG
		 ...
		 * <p>
		 * KNXnet/IP system set-up multicast address. Value is fixed to
		 * 224.0.23.12.
		 */
		// PDT,DPT: PDT_UNSIGNED_LONG
		 ...
		 * <p>
		 * Various KNXnet/IP device status info, like KNX or IP network
		 * connection failure.<br>
		 * Compatibility note: the identifier is synonymous to "EIBnet/IP Device
		 * State" in the KNX specification.
		 */
		// PDT,DPT: PDT_UNSIGNED_CHAR
		 ...
		 * Compatibility note: the identifier is synonymous to "EIBnet/IP
		 * Routing Capabilities" in the KNX specification.
		 */
		// PDT,DPT: PDT_UNSIGNED_CHAR
	...
	/**
	 * Provides an interface to load property definitions from a resource, and
	 * store property definitions into a resource.
	 * <p>
	 * It is used by the property client when loading or saving of property
	 * definitions is requested.
	 * <p>
	 * To allow the property client handling a user defined property resource,
	 * set a new property resource handler with
	 * {@link PropertyClient#setResourceHandler (tuwien.auto.calimero.mgmt.PropertyClient.ResourceHandler)}.<br>
	 * It is not necessary for subtypes implementing this interface to
	 * synchronize the methods for concurrent access.
	 * 
	 * @author B. Malinowsky
	 */
	public static interface ResourceHandler {
		 ...
		 * 
		 * @param resource
		 *            the identifier of the resource used for loading the
		 *            properties
		 ...
		 * 
		 * @param resource
		 *            the identifier of the resource used for saving the
		 *            properties
		 * @param properties
		 *            the property definitions in a collection holding
	 ...
	 * <p>
	 * A key consists of the interface object type and the property identifier
	 * of the associated property. If the property is defined globally, the
	 * global object type {@link PropertyClient.PropertyKey#GLOBAL_OBJTYPE} is
	 * used.
	 * 
	 * @author B. Malinowsky
	 */
	...
	/**
	 * Stores property definition information of one property, used for type
	 * translation and property lookup by a property client.
	 * <p>
	 * 
	 * @author B. Malinowsky
	 */
	public static class Property {
		 ...
		 * @param propertyName
		 *            property name, a friendly readable name for the property
		 * @param objectType
		 *            object type the property belongs to
		 * @param pdt
		 *            property data type
		 * @param dpt
		 *            datapoint type, use <code>null</code> if no DPT specified
		 *            or to indicate default DPT usage
		 */
		public Property(final int pid, final String pidName, final String propertyName, final int objectType,
				final int pdt, final String dpt) {
			id = pid;
			name = pidName;
		...
		/**
		 * Returns the property friendly name, a more readable name of the
		 * property.
		 * <p>
		 * 
	...
	// helper flag to determine local DM mode, mainly for detecting absence of
	// PDT
	...
	/**
	 * Creates a new property client using the specified adapter for accessing
	 * device properties.
	 * <p>
	 * The property client obtains ownership of the adapter.<br>
	 * The log service used by this property client is named "PC " +
	 * adapter.getName().
	 * 
	 * @param adapter
	...
	/**
	 * Sets a new default property resource handler used in the load / save
	 * methods by all property clients.
	 * <p>
	 * By default, a xml property file handler is used.
	 * 
	 * @param handler
	 *            the new property storage handler, or <code>null</code> to use
	 *            the client default handler
	 */
	 ...
	 * @throws KNXException
	 *             on errors in the property resource handler
	 */
	...
	/**
	 * Saves all loaded property definitions in the property client to a
	 * resource using the global set {@link ResourceHandler}.
	 * <p>
	 * 
	 * @param resource
	...
	/**
	 * Returns the property definitions used by property clients, if definitions
	 * were loaded.
	 * <p>
	 * The returned map is synchronized and a reference to the one used by all
	 * property clients. Property definitions might be added or removed as
	 * required. Note that modifications will influence lookup behavior of all
	 * property client instances.<br>
	 * A map key is of type {@link PropertyKey}, a map value is of type
	 * {@link Property}.
	 * 
	...
	/**
	 * Sets one element of a property, with the value given as string
	 * representation.
	 * <p>
	 * The value is translated according the associated property data type.
	 * 
	 * @param objIndex
	 *            interface object index in the device
	 * @param pid
	 *            property identifier
	 * @param start
	 ...
	 */
	public void setProperty(int objIndex, int pid, int start, String value) throws KNXException {
	...
	/**
	 * Gets the first property element using the associated property data type
	 ...
	 *            property identifier
	 * @param start
	 *            index of the first array element to set
	 * @param elements
	 *            number of elements to set in the property
	 * @param data
	 *            byte array holding the element data
	 * @throws KNXException
	 *             on adapter errors while setting the property elements
	 */
	public void setProperty(int objIndex, int pid, int start, int elements, byte[] data) throws KNXException {
		try {
	 ...
	 *            property identifier
	 * @param start
	 *            index of the first array element to get
	 * @param elements
	 *            number of elements to get in the property
	 * @return byte array holding the retrieved element data
	 * @throws KNXException
	 *             on adapter errors while querying the property element
	 */
	public byte[] getProperty(int objIndex, int pid, int start, int elements) throws KNXException {
		try {
	...
	/**
	 * Gets one or more elements of a property with the returned data set in a
	 * DPT translator of the associated data type.
	 * <p>
	 * 
	 * @param objIndex
	 *            interface object index in the device
	 ...
	 * If a description read is done using a property identifier (PID) like in
	 * this method, the description response is not required to contain the
	 * correct property index associated with the PID, even though recommended.
	 * The default index is 0 then.
	 * 
	 * @param objIndex
	 *            interface object index in the device
	 * @param pid
	 ...
	 * @throws KNXException
	 *             on adapter errors while querying the description
	 */
	 ...
	 *             on adapter errors while querying the description
	 */
	public Description getDescriptionByIndex(int objIndex, int propIndex) throws KNXException {
		try {
	...
	/**
	 * Does a property description scan of the properties in all interface
	 * objects.
	 * <p>
	 * 
	 * @param allProperties
	 *            <code>true</code> to scan all property descriptions in the
	 *            interface objects, <code>false</code> to only scan the object
	 *            type descriptions, i.e. ({@link PID#OBJECT_TYPE})
	 * @return a list containing the property descriptions of type
	 *         {@link Description}
	 * @throws KNXException
	...
	/**
	 * Does a property description scan of the properties of one interface
	 * object.
	 * <p>
	 * 
	 * @param objIndex
	 ...
	 * <p>
	 * 
	 * @return <code>true</code> on open adapter, <code>false</code> on closed
	 *         adapter
	 */
	...
	private DPTXlator createTranslator(int objIndex, int pid) throws KNXException {
		...
		final KNXException e = new KNXException(
				"no translator available for PID 0x" + Integer.toHexString(pid) + ", " + getObjectTypeName(ot));
	...
	private static class XMLPropertyHandler implements ResourceHandler {
				...
				while (r.read() != XMLReader.END_DOC) {
					...
					if (r.getPosition() == XMLReader.START_TAG) {
						if (e.getName().equals(OBJECT_TAG)) {
							...
							r.complete(e);
							list.add(new Property(toInt(e.getAttribute(PID_ATTR)), e.getAttribute(PIDNAME_ATTR),
									e.getAttribute(NAME_ATTR), objType, toInt(e.getAttribute(PDT_ATTR)),
									e.getAttribute(DPT_ATTR)));
						}
					} else if (r.getPosition() == XMLReader.END_TAG && e.getName().equals(PROPDEFS_TAG))
						break;
				}
				return list;
			} catch (final KNXFormatException e) {
		...
		public void save(String resource, Collection<Property> properties) throws KNXException {
			...
			try {
				...
				w.writeComment("Calimero " + Settings.getLibraryVersion() + " KNX property definitions, saved on "
						+ new Date().toString());
