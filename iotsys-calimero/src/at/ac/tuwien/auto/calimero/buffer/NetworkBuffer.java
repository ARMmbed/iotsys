 ...
 * <p>
 * Reasons to do this might be to lower the response time when answering
 * frequently occurring application queries, leading to a better runtime
 * performance. Another use would be to enable user polled applications.
 * <p>
 * A network buffer contains one or more {@link Configuration}s, each of it
 * maintains a setting how to handle, i.e. filter and buffer, certain messages.
 * In other words, a network buffer obtains knowledge of buffering messages by
 * adding configurations.<br>
 * A configuration works with filters set by the user for that configuration. A
 * filter - depending on its filter type - either applies individual filter
 * rules on incoming messages ({@link Configuration.NetworkFilter}) or answers
 * requests from users or components working with that configuration
 * ({@link Configuration.RequestFilter}).<br>
 * If a created configuration is activated and no network filter is set, a
 * default filter is used, which simply accepts all cEMI L-Data.<br>
 * If no request filter is set, no buffer lookup is done on requests, instead
 * the request is forwarded directly to the KNX network.
 * <p>
 * In general, one network buffer is created for one KNX installation, to easier
 * distinguish between different installations. Nevertheless, this is not
 * enforced in any way; a new configuration also might just always use a new
 * network buffer.
 * 
 * @author B. Malinowsky
 */
public final class NetworkBuffer {
	 ...
	 * <p>
	 * To identify the buffer an unique installation identifier can be given
	 * through <code>installationID</code>.<br>
	 * If <code>null</code> or an empty string is supplied for the installation
	 * ID, a new default ID is generated of the form "Installation [ID]", where
	 * [ID] is an unique incrementing number. Note, the installation ID string
	...
	public static synchronized NetworkBuffer createBuffer(String installationID) {
		if (getBuffer(installationID) != null)
			throw new KNXIllegalArgumentException("buffer \"" + installationID + "\" already exists");
	 ...
	 * {@link NetworkBuffer#removeConfiguration (at.ac.tuwien.auto.calimero.buffer.Configuration)}
	 * is called.
	 * 
	 * @param installationID
		...
		for (final Iterator i = buffers.iterator(); i.hasNext();) {
			...
			if (b.inst.equals(installationID)) {
				...
				while (!b.configs.isEmpty())
					b.removeConfiguration((Configuration) b.configs.get(b.configs.size() - 1));
	...
	/**
	 * Creates a new configuration for the network buffer identified by the
	 * installation ID.
	 * <p>
	 * The configuration is added to the network buffer specified by the
	 * installation ID. If a network buffer with the supplied installation ID
	 * does not exist, it will be created. If <code>null</code> or an empty
	 * string is supplied for the installation ID, a new default ID is generated
	 * (see {@link NetworkBuffer#createBuffer(String)}. <br>
	 * If the supplied <code>link</code> gets closed, the created configuration
	 * will get deactivated (see {@link Configuration#activate(boolean)}), and
	 * the buffered link of the configuration, obtained with
	 * {@link Configuration#getBufferedLink()}, will get closed as well.
	 * 
	 * @param link
	 ...
	 */
	public static Configuration createConfiguration(KNXNetworkLink link, String installationID) {
	...
	/**
	 * Removes a configuration from the network buffer identified by the
	 * installation ID.
	 * <p>
	 * If <code>installationID</code> is <code>null</code>, all network buffers
	 * are searched for the configuration <code>c</code>.<br>
	 ...
	 * <p>
	 * The configuration is deactivated and will not receive any further events
	 * or incoming messages from the base network link supplied at creation of
	 * that configuration.
	 * 
	 * @param c
	 *            the configuration to remove
