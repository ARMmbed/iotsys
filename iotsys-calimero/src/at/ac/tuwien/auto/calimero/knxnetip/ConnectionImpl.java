...
abstract class ConnectionImpl implements KNXnetIPConnection {
	// IMPLEMENTATION NOTE: on MS Windows platforms, interruptible I/O is not
	// supported,
	...
	/**
	 * Status code of communication: in idle state, received an acknowledge
	 * error in response, ready to send.
	 * <p>
	 */
	public static final int ACK_ERROR = 3;

	/**
	 * Status code of communication: waiting for confirmation after acknowledge,
	 * no error, not ready to send.
	 * <p>
	 */
	...
	// KNXnet/IP client SHALL wait 10 seconds for a connect response frame from
	// server
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.knxnetip.KNXnetIPConnection#addConnectionListener
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.knxnetip.KNXnetIPConnection#removeConnectionListener
	 * (tuwien.auto.calimero.KNXListener)
	 */
	 ...
	 * calls from different threads is being maintained according to invocation
	 * order (FIFO).<br>
	 * A call of this method blocks until (possible) previous invocations are
	 * done, then does communication according to the protocol and waits for
	 * response (either ACK or cEMI confirmation), timeout or an error
	 * condition.<br>
	 * Note that, for now, when using blocking mode any ongoing nonblocking
	 * invocation is not detected or considered for waiting until completion.
	 * <p>
	 * If mode is {@link KNXnetIPConnection#NONBLOCKING}, sending is only
	 * permitted if no other send is currently being done, otherwise throws a
	 * {@link KNXIllegalStateException}. A user has to check the state
	...
	public void send(CEMI frame, BlockingMode mode) throws KNXTimeoutException, KNXConnectionClosedException {
		...
		synchronized (lock) {
			if (mode == NONBLOCKING && state != OK && state != ACK_ERROR) {
				logger.warn("nonblocking send invoked while waiting for data response " + "in state " + state
						+ " - aborted");
			...
			try {
				...
				if (serviceRequest == KNXnetIPHeader.ROUTING_IND)
					...
					buf = PacketHelper.toPacket(new ServiceRequest(serviceRequest, channelID, getSeqNoSend(), frame));
	 ...
	 * <p>
	 * The communication state of this object is assumed to be closed state.
	 * This method is designed to be called only once during the objects
	 * lifetime!
	 * 
	 * @param localEP
	 *            the local endpoint to use for communication channel
	 * @param serverCtrlEP
	 *            the remote server control endpoint used for connect request
	 * @param cri
	 *            connect request information used to configure the
	 *            communication attributes
	 * @param useNAT
	 *            <code>true</code> to use a NAT (network address translation)
	 *            aware communication mechanism, <code>false</code> to use the
	 *            default way
	 * @throws KNXException
	 ...
	 *             if connect response is in wrong format
	 */
	protected void connect(InetSocketAddress localEP, InetSocketAddress serverCtrlEP, CRI cri, boolean useNAT)
			throws KNXException {
		ctrlEP = serverCtrlEP;
		isNatAware = useNAT;
		...
		try {
			...
			final HPAI hpai = new HPAI(HPAI.IPV4_UDP,
					isNatAware ? null : (InetSocketAddress) socket.getLocalSocketAddress());
		...
		if (!changed)
			...
			e = new KNXRemoteException("acknowledge error, failed to connect to control endpoint " + ctrlEP);
		else
	...
	/**
	 * Increments the protocol's receive sequence number, with increment on
	 * sequence number 255 resulting in 0.
	 * <p>
	 */
	...
	/**
	 * Increments the protocol's send sequence number, with increment on
	 * sequence number 255 resulting in 0.
	 * <p>
	 */
	 ...
	 * ({@link KNXListener#frameReceived(FrameEvent)}) for the supplied cEMI
	 * <code>frame</code>.
	 * 
	 * @param frame
	 *            the cEMI to generate the event for
	 */
	...
	void close(int initiator, String reason, LogLevel level, Throwable t) {
		...
		try {
			...
			// we have to do this strange catch here, since if socket already
			// failed
			// before close(), getLocalSocketAddress() might throw illegal
			// argument
			// exception or return the wildcard address, indicating a messed up
			// socket
	...
	// acquire and its associated release don't have to be invoked by same
	// thread
	...
	private final class HeartbeatMonitor extends Thread {
		// client SHALL wait 10 seconds for a connection state response from
		// server
		...
		public void run() {
			final byte[] buf = PacketHelper.toPacket(new ConnectionstateRequest(channelID,
					new HPAI(HPAI.IPV4_UDP, isNatAware ? null : (InetSocketAddress) socket.getLocalSocketAddress())));
			final DatagramPacket p = new DatagramPacket(buf, buf.length, ctrlEP.getAddress(), ctrlEP.getPort());
			try {
				while (true) {
	...
	private final class Receiver extends Thread {
				...
				while (!quit) {
					...
					try {
						...
						if (h.getTotalLength() > p.getLength())
							...
							// so unused service types of us can stay 0 by
							// default
							logger.warn("received frame with service type 0 - ignored");
						else
							parseService(h, data, p.getOffset() + h.getStructLength(), p.getAddress(), p.getPort());
		...
		void quit() {
			// since receiver might not get interrupted, we additionally set
			// quit flag
		...
		private void parseService(KNXnetIPHeader h, byte[] data, int offset, InetAddress src, int port)
			...
			if (svc == KNXnetIPHeader.CONNECT_REQ)
				...
				if (res.getStatus() == ErrorCodes.NO_ERROR && ep.getHostProtocol() == HPAI.IPV4_UDP) {
					...
					// empty, we fall back to the IP address and port of the
					// sender
				...
				if (res.getChannelID() != channelID)
					logger.warn("received wrong acknowledge channel-ID " + res.getChannelID() + ", expected "
							+ channelID + " - ignored");
				else if (res.getSequenceNumber() != getSeqNoSend())
					logger.warn("received invalid acknowledge send-sequence " + +res.getSequenceNumber() + ", expected "
							+ getSeqNoSend() + " - ignored");
				else {
					if (!checkVersion(h))
						return;
					incSeqNoSend();
					...
					if (internalState == ACK_ERROR)
						logger.warn("received acknowledge status: " + res.getStatusString());
				}
			} else
				handleService(h, data, offset);
		}
		 ...
		 * {@link ConnectionImpl#close(int, String, LogLevel, Throwable)} is
		 * invoked.
		 * 
		 * @param h
		 *            KNX header to check
