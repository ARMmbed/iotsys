...
public class KNXnetIPDevMgmt extends ConnectionImpl {
	 ...
	 */
	public KNXnetIPDevMgmt(InetSocketAddress localEP, InetSocketAddress serverCtrlEP, boolean useNAT)
			throws KNXException {
	...
	/**
	 * Sends a cEMI device management frame to the remote server communicating
	 * with this endpoint.
	 * <p>
	 * 
	 * @param frame
	 *            cEMI device management message of type {@link CEMIDevMgmt} to
	 *            send
	 */
	...
	void handleService(KNXnetIPHeader h, byte[] data, int offset) throws KNXFormatException, IOException {
		...
		if (svc == KNXnetIPHeader.DEVICE_CONFIGURATION_REQ) {
			...
			try {
				...
				// try to get at least the connection header of the service
				// request
			...
			if (req.getChannelID() == getChannelID() && seq == getSeqNoRcv()) {
				...
				final byte[] buf = PacketHelper
						.toPacket(new ServiceAck(KNXnetIPHeader.DEVICE_CONFIGURATION_ACK, getChannelID(), seq, status));
				...
				logger.warn("received dev.mgmt request channel-ID " + req.getChannelID() + ", receive-sequence " + seq
						+ ", expected " + getSeqNoRcv() + " - ignored");
		} else
