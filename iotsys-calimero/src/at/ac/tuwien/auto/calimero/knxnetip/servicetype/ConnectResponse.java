 ...
 * <p>
 * The connect response is sent in answer to a connect request to inform the
 * client about the connection status and to supply necessary information.<br>
 * It is addressed to the client's control endpoint.<br>
 * Note, that only on a successful connect response, as indicated by the
 * response status, the communication channel ID, connect response data and
 * server data endpoint carry valid data.
 * <p>
...
public class ConnectResponse extends ServiceType {
	 ...
	 * 
	 * @param data
	 *            byte array containing a connect response structure
	 * @param offset
	 *            start offset of response in <code>data</code>
	 * @throws KNXFormatException
	 *             if no connect response was found or invalid structure
	 */
	 ...
	 * 
	 * @param status
	 *            status code giving information of refusal to the corresponding
	...
	/**
	 * Creates a connect response with full customization of response
	 * information.
	 * <p>
	 * 
	 * @param channelID
	 *            communication channel ID to use for the connection, 0 &lt;= id
	 *            &lt;= 255
	 * @param status
	 *            status code information to the corresponding request, 0 &lt;=
	 *            status &lt;= 255
	 * @param dataEndpoint
	 *            data endpoint of the server use for communication
	 * @param responseData
	 *            connection type response data
	 */
	...
	/**
	 * Returns the communication channel ID on a successful established
	 * connection.
	 * <p>
	...
	/**
	 * Returns the connection response data supplied in the response on a
	 * successful established connection.
	 * <p>
	 * The CRD is <code>null</code> on error, check the status code of the
	 * response before.
	 * 
	...
	/**
	 * Returns the server data endpoint used for the communication on a
	 * successful established connection.
	 * <p>
	 * The HPAI is <code>null</code> otherwise, check the status code of the
	 * response.
	 * 
