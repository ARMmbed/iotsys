 ...
 * <p>
 * A KNXnet/IP router is a fast replacement for line/backbone couplers and
 * connected main/backbone lines, using Ethernet cabling for example.<br>
 * The router use point to multipoint communication (multicast). By default,
 * routers are joined to the {@link KNXnetIPRouter#DEFAULT_MULTICAST} multicast
 * group. On more KNX installations in one IP network, different multicast
 * addresses have to be assigned.<br>
 * All IP datagrams use port number 3671, i.e only datagrams on this port are
 * observed. <br>
 * The routing protocol is an unconfirmed service.
 * <p>
 * Optionally, a listener of type {@link RouterListener} can be supplied to
 * {@link #addConnectionListener(KNXListener)} instead of a default
 * {@link KNXListener}, to receive
 * {@link RouterListener#lostMessage(LostMessageEvent)} notifications.
 * <p>
 * Multicast considerations:<br>
 * By default, the loopback mode of the socket used for sending multicast
 * datagrams is disabled.
 * <p>
 * A multicast datagram sent with an initial hop count greater 1 may be
 * delivered to the sending host on a different interface (than the sending
 * one), if the host is a member of the multicast group on that interface. The
 * loopback mode setting of the sender's socket has no effect on this behavior.
 * 
 * @author B. Malinowsky
 */
public class KNXnetIPRouter extends ConnectionImpl {
	 ...
	 * <p>
	 * In general, routers are assigned a multicast address by adding an offset
	 * to the system setup multicast address ({@value #DEFAULT_MULTICAST}) for
	 * each KNX installation, by default this offset is 0 (i.e. only one used
	 * installation).
	 * 
	 * @param netIf
	 *            specifies the local network interface used to join the
	 *            multicast group and send outgoing multicast data, use
	 *            <code>null</code> to use the default interface; useful for
	 *            multihomed hosts
	 * @param mcGroup
	 *            address of the multicast group this router is joined to, or
	 *            <code>null</code> to use the default multicast
	 *            ({@value #DEFAULT_MULTICAST}); value of <code>mcAddr</code> >=
	 *            {@value #DEFAULT_MULTICAST}
	 * @throws KNXException
	 *             on socket error, or if joining to group failed
	 */
	public KNXnetIPRouter(NetworkInterface netIf, InetAddress mcGroup) throws KNXException {
	 ...
	 * @param mode
	 *            arbitrary value, does not influence behavior, since routing is
	 *            always a unconfirmed, nonblocking service
	 */
	...
	/**
	 * Sets the default hop count (TTL) used in the IP header of encapsulated
	 * cEMI messages.
	 * <p>
	 * This value is used to limit the multicast geographically, although this
	 * is just a rough estimation. The hop count value is forwarded to the
	 * underlying multicast socket used for communication.
	 * 
	 * @param hopCount
	 *            hop count value, 0 &lt;= value &lt;= 255
	 */
	...
	/**
	 * Returns the default hop count (TTL) used in the IP header of encapsulated
	 * cEMI messages.
	 * <p>
	 ...
	 */
	void handleService(KNXnetIPHeader h, byte[] data, int offset) throws KNXFormatException {
