 ...
 * <p>
 * The tunneling protocol specifies a point-to-point exchange of KNX frames over
 * an IP network connection between two KNXnet/IP devices - client and
 * server.<br>
...
public class KNXnetIPTunnel extends ConnectionImpl {
	/**
	 * Connection type used to tunnel between two KNXnet/IP devices (client /
	 * server).
	 * <p>
	 */
	...
	/**
	 * Tunneling on busmonitor layer, establishes a busmonitor tunnel to the KNX
	 * network.
	 * <p>
	 */
	...
	/**
	 * Tunneling on link layer, establishes a link layer tunnel to the KNX
	 * network.
	 * <p>
	 */
	 ...
	 * <p>
	 * Establishing a raw tunneling layer ({@link #RAW_LAYER}) is not supported
	 * yet.<br>
	 * 
	 * @param KNXLayer
	 *            KNX tunneling layer (e.g. {@link #LINK_LAYER})
	 * @param localEP
	 *            specifies the local endpoint with the socket address to be
	 *            used by the tunnel
	 * @param serverCtrlEP
	 *            control endpoint of the server to establish connection to
	 * @param useNAT
	 ...
	 */
	public KNXnetIPTunnel(short KNXLayer, InetSocketAddress localEP, InetSocketAddress serverCtrlEP, boolean useNAT)
			throws KNXException {
	 ...
	 * 
	 * @param frame
	 *            cEMI message to send, the expected cEMI type is according to
	 *            the used tunneling layer
	 */
	...
	void handleService(KNXnetIPHeader h, byte[] data, int offset) throws KNXFormatException, IOException {
		...
		if (svc == KNXnetIPHeader.TUNNELING_REQ) {
			...
			try {
				...
				// check if at least the connection header of the service
				// request
			...
			if (req.getChannelID() != getChannelID()) {
				logger.warn("received wrong request channel-ID " + req.getChannelID() + ", expected " + getChannelID()
						+ " - ignored");
				return;
			}
			final short seq = req.getSequenceNumber();
			if (seq == getSeqNoRcv() || seq + 1 == getSeqNoRcv()) {
				final short status = h.getVersion() == KNXNETIP_VERSION_10 ? ErrorCodes.NO_ERROR
						: ErrorCodes.VERSION_NOT_SUPPORTED;
				final byte[] buf = PacketHelper
						.toPacket(new ServiceAck(KNXnetIPHeader.TUNNELING_ACK, getChannelID(), seq, status));
				...
				logger.warn("tunneling request invalid receive-sequence " + seq + ", expected " + getSeqNoRcv());
			if (seq == getSeqNoRcv()) {
				incSeqNoRcv();
				final CEMI cemi = req.getCEMI();
				// leave if we are working with an empty (broken) service
				// request
				if (cemi == null)
					return;
				if (cemi.getMessageCode() == CEMILData.MC_LDATA_IND
					...
					// invariant: notify listener before return from blocking
					// send
					fireFrameReceived(cemi);
					setStateNotify(OK);
