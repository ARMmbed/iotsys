 ...
 */
class CRBase {
	private static final LogService logger = LogManager.getManager().getLogService("KNXnet/IP service");
	 ...
	 * 
	 * @param data
	 *            byte array containing a CRI or CRD structure
	 * @param offset
	 *            start offset
	 ...
	 * <p>
	 * The array of <code>optionalData</code> is not copied for internal
	 * storage. No additional checks regarding content are done.
	 * 
	 * @param connectionType
	 *            connection type the CR is used for
	 * @param optionalData
	 *            byte array containing optional host protocol independent and
	 *            dependent data, this information is located starting at offset
	 *            2 in the CR structure, <code>optionalData.length</code> < 254
	 */
	...
	static CRBase create(boolean request, short type, byte[] data) {
		...
		if (type != KNXnetIPDevMgmt.DEVICE_MGMT_CONNECTION)
			logger.warn("can't deduce specific CR from connection type - create default");
		return request ? (CRBase) new CRI(type, (byte[]) opt.clone()) : new CRD(type, (byte[]) opt.clone());
	}
	...
	/**
	 * Returns a textual representation of the connection type, length and
	 * optional data.
	 * <p>
	 * 
 ...
 * <p>
 * The CRI structure is used for the additional information in a connection
 * request.<br>
 * It is built up of host protocol independent data and host protocol dependent
 * data, both optional. Refer to the available subtypes for more specific type
 * information.
 * <p>
 * For now, a plain CRI is returned for management connections, since this
 * connection type doesn't require any additional host protocol data.
 * <p>
...
public class CRI extends CRBase {
	 ...
	 * 
	 * @param connectionType
	 *            connection type the CRI is used for (e.g. tunnel connection)
	 * @param optionalData
	 *            byte array containing optional host protocol independent and
	 *            dependent data, this information is located starting at offset
	 *            2 in the CRI structure, <code>optionalData.length</code> < 254
	 */
	 ...
	 * Creates a new CRI out of a byte array.
	 * <p>
	 * If possible, a matching, more specific, CRI subtype is returned. Note,
	 * that CRIs for specific communication types might expect certain
	 * characteristics on <code>data</code> (regarding contained data).<br>
	 * 
	 * @param data
	 *            byte array containing the CRI structure
	 * @param offset
	 *            start offset of CRI in <code>data</code>
	 ...
	 * If possible, a matching, more specific, CRI subtype is returned. Note,
	 * that CRIs for specific communication types might expect certain
	 * characteristics on <code>optionalData</code> (regarding length and/or
	 * content).<br>
	 * 
	 * @param connectionType
	 *            connection type this CRI is used for (e.g. tunnel connection)
	 * @param optionalData
	 *            byte array containing optional host protocol independent and
	 *            dependent data, this information is located starting at offset
	 *            2 in the CRI structure, <code>optionalData.length</code> <
	 *            254, may be <code>null</code> for no optional data
	 * @return the new CRI object
	 */
