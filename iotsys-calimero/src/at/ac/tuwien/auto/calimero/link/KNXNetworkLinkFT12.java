...
/**
 * Implementation of the KNX network network link based on the FT1.2 protocol,
...
public class KNXNetworkLinkFT12 implements KNXNetworkLink {
	private static final class LinkNotifier extends EventNotifier {
		...
		public void frameReceived(FrameEvent e) {
			try {
				final CEMILData f = (CEMILData) CEMIFactory.createFromEMI(e.getFrameBytes());
	...
	/**
	 * Creates a new network link based on the FT1.2 protocol for accessing the
	 * KNX network.
	 * <p>
	 ...
	 * @throws KNXException
	 */
	public KNXNetworkLinkFT12(String portID, KNXMediumSettings settings) throws KNXException {
	...
	public String toString() {
		return getName() + (closed ? "(closed), " : ", ") + medium.getMediumString() + " medium hopcount " + hopCount;
	}
	...
	private byte[] createEMI(CEMILData f) {
		final byte[] buf = createEMI(f.getMessageCode(), f.getDestination(), f.getPriority(), f.getPayload());
