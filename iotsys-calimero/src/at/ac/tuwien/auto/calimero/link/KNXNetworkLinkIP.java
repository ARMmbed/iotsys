...
/**
 * Implementation of the KNX network link based on the KNXnet/IP protocol, using
 * a {@link KNXnetIPConnection}.
 * <p>
 * Once a link has been closed, it is not available for further link
 ...
 * unconfirmed protocol. This implies that no confirmation frames are generated,
 * thus {@link NetworkLinkListener#confirmation(FrameEvent)} is not used.
 * <p>
 * IP address considerations:<br>
 * On more IP addresses assigned to the local host (on possibly several local
 * network interfaces), the default chosen local host address can differ from
 * the expected. In this situation, the local endpoint has to be specified
 * manually during instantiation. <br>
 * Network Address Translation (NAT) aware communication can only be used, if
 * the KNXnet/IP server of the remote endpoint supports it. Otherwise,
 * connection timeouts will occur. With NAT enabled, KNXnet/IP accepts IPv6
 * addresses. By default, the KNXnet/IP protocol only works with IPv4
 * addresses.<br>
 * 
 * @author B. Malinowsky
 */
public class KNXNetworkLinkIP implements KNXNetworkLink {
	 ...
	 * 
	 * @param serviceMode
	 *            mode of communication to open, <code>serviceMode</code> is one
	 *            of the service mode constants (e.g. {@link #TUNNEL});
	 *            depending on the mode set, the expected local / remote
	 *            endpoints might differ
	 * @param localEP
	 *            the local endpoint of the link to use;<br>
	 *            - in tunneling mode (point-to-point), this is the client
	 *            control endpoint, use <code>null</code> for the default local
	 *            host and an ephemeral port number<br>
	 *            - in {@link #ROUTER} mode, specifies the multicast interface,
	 *            i.e. the local network interface is taken that has the IP
	 *            address bound to it (if IP address is bound more than once,
	 *            it's undefined which interface is returned), the port is not
	 *            used; use <code>null</code> for <code>localEP</code> or an
	 *            unresolved IP address to take the host's default multicast
	 *            interface
	 * @param remoteEP
	 *            the remote endpoint of the link to communicate with;<br>
	 *            - in tunneling mode (point-to-point), this is the server
	 *            control endpoint <br>
	 *            - in {@link #ROUTER} mode, the IP address specifies the
	 *            multicast group to join, the port is not used; use
	 *            <code>null</code> for <code>remoteEP</code> or an unresolved
	 *            IP address to take the default multicast group
	 * @param useNAT
	 *            <code>true</code> to use network address translation in
	 *            tunneling service mode, <code>false</code> to use the default
	 *            (non aware) mode; parameter is ignored for routing
	 * @param settings
	 *            medium settings defining device and medium specifics needed
	 *            for communication
	 * @throws KNXException
	 *             on failure establishing link using the KNXnet/IP connection
	 */
	public KNXNetworkLinkIP(int serviceMode, InetSocketAddress localEP, InetSocketAddress remoteEP, boolean useNAT,
	...
	/**
	 * Creates a new network link based on the KNXnet/IP tunneling protocol,
	 * using a {@link KNXnetIPTunnel} with default communication settings.
	 * <p>
	 * The link is established using a KNXnet/IP tunnel, the local endpoint is
	 * the default local host, the remote endpoint uses the default KNXnet/IP
	 * port number, network address translation (NAT) is disabled.
	 * 
	 * @param remoteHost
	 *            remote host name
	 * @param settings
	 ...
	 *             on failure establishing link using the KNXnet/IP connection
	 */
	public KNXNetworkLinkIP(String remoteHost, KNXMediumSettings settings) throws KNXException {
		this(TUNNEL, null, new InetSocketAddress(remoteHost, KNXnetIPConnection.IP_PORT), false, settings);
	}

	/**
	 * Creates a new network link based on the KNXnet/IP routing protocol, using
	 * a {@link KNXnetIPRouter}.
	 * <p>
	 * 
	 * @param netIf
	 *            local network interface used to join the multicast group and
	 *            for sending, use <code>null</code> for the host's default
	 *            multicast interface
	 * @param mcGroup
	 *            address of the multicast group to join, use <code>null</code>
	 *            for the default KNXnet/IP multicast address
	 * @param settings
	 *            medium settings defining device and medium specifics needed
	 *            for communication
	 * @throws KNXException
	 *             on failure establishing link using the KNXnet/IP connection
	 */
	public KNXNetworkLinkIP(NetworkInterface netIf, InetAddress mcGroup, KNXMediumSettings settings)
			throws KNXException {
	...
	/**
	 * {@inheritDoc} When communicating with a KNX network which uses open
	 * medium, messages are broadcasted within domain (as opposite to system
	 * broadcast) by default. Specify <code>dst null</code> for system
	 * broadcast.
	 */
	...
	public String toString() {
		return getName() + (mode == TUNNEL ? " tunnel" : " routing") + " mode" + (closed ? " (closed), " : ", ")
				+ medium.getMediumString() + " medium hopcount " + hopCount;
	}

	private CEMILData adjustMsgType(CEMILData msg) {
		final boolean srcOk = msg.getSource().getRawAddress() != 0;
		// just return if we don't need to adjust source address and don't need
		// LDataEx
		if ((srcOk || medium.getDeviceAddress().getRawAddress() == 0)
				&& (medium instanceof TPSettings || msg instanceof CEMILDataEx))
			return msg;
		return (CEMILDataEx) CEMIFactory.create(srcOk ? null : medium.getDeviceAddress(), null, msg, true);
	}
	...
	private void doSend(CEMILData msg, boolean waitForCon) throws KNXTimeoutException, KNXLinkClosedException {
		if (closed)
		...
		if (medium instanceof PLSettings) {
			...
			if (f.getAdditionalInfo(CEMILDataEx.ADDINFO_RFMEDIUM) == null) {
				...
				// add-info: rf-info (0 ignores a lot), sn (6 bytes), lfn
				// (=255:void)
				f.addAdditionalInfo(CEMILDataEx.ADDINFO_RFMEDIUM,
						new byte[] { 0, sn[0], sn[1], sn[2], sn[3], sn[4], sn[5], (byte) 0xff });
				logger.trace("send - added RF additional info to message "
						+ (f.isDomainBroadcast() ? "(domain address)" : "(s/n)"));
			}
		}
		try {
			logger.info("send message to " + msg.getDestination() + (waitForCon ? ", wait for confirmation" : ""));
