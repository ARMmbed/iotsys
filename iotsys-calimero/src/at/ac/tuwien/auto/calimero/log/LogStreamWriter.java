 ...
 * <p>
 * An existing output stream has to be supplied on creation of this LogWriter.
 * All output will be checked against the internal log level, on logging
 * permitted the output is formatted (optional) and handed to the underlying
 * writer.<br>
 * Output is not automatically flushed after each write by default.<br>
 * Using <code>autoFlush = true</code> in the constructor ensures that no data
 * buffering will delay the output. Note that this may degrade performance.<br>
...
public class LogStreamWriter extends LogWriter {
	 ...
	 * {@link #formatOutput(String, LogLevel, String, Throwable)
	 * formatOutput(LogLevel, String, Throwable)} before writing out the log
	 * message. Defaults to <code>true</code>, but might be set to
	 * <code>false</code> by subtypes if message given to <code>write</code> is
	 * already formatted.
	 */
	protected boolean formatOutput = true;

	/**
	 * Line separator, retrieved from the property "line.separator" and set in
	 * the constructor.
	 */
	...
	/**
	 * Sets line separator; also called by subtypes creating the output stream
	 * on their own.
	 */
	 ...
	 * 
	 * @param os
	 *            an OutputStream used by this LogStreamWriter
	 */
	...
	/**
	 * Creates a <code>LogStreamWriter</code> with specified log level and
	 * output stream.
	 * 
	 * @param level
	 *            log level assigned with this <code>LogStreamWriter</code>
	 * @param os
	 *            an OutputStream used by this <code>LogStreamWriter</code>
	...
	/**
	 * Creates a <code>LogStreamWriter</code> with specified log level and
	 * output stream. Parameter <code>autoFlush</code> sets flushing behavior on
	 * write() calls.
	 * 
	 * @param level
	 *            log level assigned with this <code>LogStreamWriter</code>
	 ...
	 *            an OutputStream used by this <code>LogStreamWriter</code>
	 * @param autoFlush
	 *            flush output after every successful call to write()
	 ...
	 * (non-Javadoc)
	 * 
	 * @see tuwien.auto.calimero.log.LogWriter#write (java.lang.String,
	...
	/**
	 * Checks if logging output with log <code>level</code> is possible and
	 * would get accepted by this <code>LogStreamWriter</code>.
	 * <p>
	 * Therefore this method also checks that the underlying output stream is
	 * opened.
	 * 
	 * @param level
	 ...
	 * The output returned by default follows the shown format. The date/time
	 * format is according ISO 8601 representation, extended format with decimal
	 * fraction of second (milliseconds):<br>
	 * "YYYY-MM-DD hh:mm:ss,sss level=<code>level.toString()</code>,
	 * <code>logService</code>: <code>msg</code>
	 * (<code>t.getMessage()</code>)"<br>
	 * or, if throwable is <code>null</code> or throwable-message is
	 * <code>null</code><br>
	 * "YYYY-MM-DD hh:mm:ss,sss logService,
	 * LogLevel=<code>level.toString()</code>: <code>msg</code>".<br>
	 * If <code>logService</code> contains '.' in the name, only the part after
	 * the last '.' will be used. This way names like "package.subpackage.name"
	 * are shortened to "name". Nevertheless, if the first character after '.'
	 * is numeric, no truncation will be done to allow e.g. IP addresses in the
	 * log service name.
	 * 
	 * @param svc
	 *            name of the log service the message comes from
	 * @param l
	 *            log level of message and throwable
	 * @param msg
	 *            message to format
	 * @param t
	 *            an optional throwable object to format, might be
	 *            <code>null</code>
	...
	private synchronized void doWrite(String logService, LogLevel level, String msg, Throwable t) {
		if (logAllowed(level))
			try {
