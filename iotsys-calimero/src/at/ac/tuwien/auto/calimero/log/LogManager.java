 ...
 * {@link #getManager()}.<br>
 * A log service can be queried and removed. A log writer can be added (i.e.
 * registered) and removed, either to a particular log service or as a global
 * log writer. A global log writer will receive all logging output from all
 * registered log services.
 * 
 * @author B. Malinowsky
 * @see LogWriter
 * @see LogService
...
public final class LogManager {
	// TODO method to provide an easy, clean way to shutdown logging at the end
	// of an
	...
	/**
	 * Checks whether a log service with <code>name</code> exists in the
	 * manager.
	 * <p>
	 * A log service is only listed in the manager, if it was initially queried
	 * using {@link #getLogService(String)}.
	 * 
	 * @param name
	 *            name of log service
	 * @return <code>true</code> if log service exists, <code>false</code>
	 *         otherwise
	 */
	 ...
	 * <p>
	 * If the log service with this name already exists in the manager, it will
	 * be returned, otherwise a new log service with this name will be created
	 * and added to the log services listed in the manager.
	 * 
	 * @param name
	 ...
	 * <p>
	 * If no log service with the specified name is found, no action is
	 * performed.
	 * 
	 * @param name
	 *            name of log service
	 ...
	 * <p>
	 * Note that the writer is added to the log service(s) regardless if it was
	 * already added before.<br>
	 * If the writer is added global, it will receive logging information from
	 * all log services that are already registered or will be registered in the
	 * future.
	 * 
	 * @param logService
	...
	/**
	 * Removes a log writer, either global or from a particular
	 * <code>logService</code>.
	 * <p>
	 * Note that for a writer to be removed global, it had to be added global
	 * before.
	 * 
	 * @param logService
	 *            name of the log service of which the writer will be removed;
	 *            to remove the writer global, use an empty string or
	 *            <code>null</code>
	 * @param writer
	 *            log writer to remove
	 ...
	 * <p>
	 * Global are all log writers which were not registered at a particular log
	 * service.
	 * 
