 ...
 * <p>
 * Note, that the most significant bit of the main group (i.e. bit 15 in the
 * unstructured address) is reserved, but not in use for now. Hence this bit is
 * not checked for, but nevertheless stored and returned by this implementation.
 */
public class GroupAddress extends KNXAddress {
	 ...
	 * 
	 * @param address
	 *            the address value in the range 0 <= value <= 0xFFFF
	 */
	...
	/**
	 * Creates a KNX group address from the 3-level notation main-, middle- and
	 * sub-group.
	 * <p>
	 * 
	 * @param mainGroup
	 *            main group value, in the range 0 <= value <= 0x1F
	 * @param middleGroup
	 *            middle group value, in the range 0 <= value <= 0x7
	 * @param subGroup
	 *            sub group value, in the range 0 <= value <= 0xFF
	 */
	...
	/**
	 * Creates a KNX group address from the 2-level notation main- and
	 * sub-group.
	 * <p>
	 * 
	 * @param mainGroup
	 *            main group value, in the range 0 <= value <= 0x1F
	 * @param subGroup
	 *            sub group value, in the range 0 <= value <= 0x7FF
	 */
	 ...
	 * representation.
	 * <p>
	 * The address might be formatted in 2- or 3-level groups. Allowed
	 * separators are '.' or '/', mutually exclusive.
	 * 
	 * @param address
	 *            string containing the KNX address
	 * @throws KNXFormatException
	 *             on unknown address type, wrong address syntax, group values
	 *             out of range, or wrong separator used
	 */
	 ...
	 *             if the xml element is no KNX address or the address couldn't
	 ...
	 * <p>
	 * Note that, since this notation of levels only affects visual presentation
	 * and not internal operation, this is a class setting.
	 * 
	 * @param format3Level
	 *            <code>true</code> for 3-level group format, <code>false</code>
	 *            for 2-level group format
	 */
	 ...
	 * <p>
	 * 
	 * @return <code>true</code> for 3-level formatting, <code>false</code>
	 *         otherwise
	 */
	 ...
	 * <p>
	 * The main group is equal for both the 2- and 3-level group address
	 * notation (see class header specification).
	 * 
	 ...
	 * <p>
	 * The middle group consists of 3 bits, starting with bit 8 to 10 in the
	 * address field.
	 * 
	 ...
	 * <p>
	 * Depending on {@link #is3LevelPresentation()}, the address is formatted in
	 * 2- or 3-level group notation.
	 * 
	 ...
	 * @param obj
	 *            KNX address object
	 * @return <code>true</code> iff <code>obj</code> is of this type and
	 *         contains the same address (raw), <code>false</code> otherwise
	 */
