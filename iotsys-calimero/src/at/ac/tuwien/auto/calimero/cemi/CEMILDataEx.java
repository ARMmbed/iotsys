 ...
 * <p>
 * Extended frame formats are supported, with a transport layer protocol data
 * unit of 255 bytes maximum. Additional information might be specified.
 * <p>
...
public class CEMILDataEx extends CEMILData implements Cloneable {
	...
	public static final class AddInfo {
		 ...
		 * 
		 * @param infoType
		 *            additional information type ID
		 * @param info
		 *            information data
		 */
		...
		/**
		 * Returns the type of additional information (see ADDINFO_* constants
		 * in class CEMILDataEx).
		 * <p>
		 * 
		 * @return type ID
		 */
	 ...
	 *            without the length field, tpdu.length &lt;= 255
	 * @param p
	 ...
	 *            &lt;= value &lt;= 7
	 */
	public CEMILDataEx(int msgCode, IndividualAddress src, KNXAddress dst, byte[] tpdu, Priority p, boolean repeat,
	 ...
	 * <p>
	 * It replaces additional information of the same type, if any was
	 * previously added.
	 * 
	 * @param infoType
	 *            type ID of additional information
	 * @param info
	...
	public synchronized void addAdditionalInfo(int infoType, byte[] info) {
		...
		if (!checkAddInfoLength(infoType, info.length))
			throw new KNXIllegalArgumentException(
					"wrong info data length, expected " + ADDINFO_LENGTHS[infoType] + " bytes");
	...
	/**
	 * Returns additional information data corresponding to the supplied type
	 * ID, if it is contained in the message.
	 * <p>
	 * 
	 * @param infoType
	 *            type ID of the request additional information
	 * @return additional information data or <code>null</code> if no such
	 ...
	 * <p>
	 * 
	 * @return <code>true</code> if this is an extended frame,
	 ...
	 * 
	 * @param domainOnly
	 *            <code>true</code> for doing a broadcast only within the
	 *            domain, <code>false</code> for a system broadcast
	 */
	 ...
	 * <p>
	 * By default, <code>true</code> is returned, indicating "domain-only"
	 * broadcast on open media or "don't care" on closed media.
	 * 
	 * @return <code>true</code> if broadcast only within domain or "don't care"
	 *         mode, <code>false</code> for system broadcast
	 */
	 ...
	 * 
	 * @param infoType
	 *            type ID of additional information to remove
	 */
	 ...
	 * 
	 * @see
	 * tuwien.auto.calimero.cemi.CEMILData#setPriority(tuwien.auto.calimero.
	 * Priority)
	 */
	...
	public Object clone() {
		try {
			...
			// the byte arrays with additional info content are used internal
			// only
	 ...
	 * (non-Javadoc)
	 * 
	 * @see tuwien.auto.calimero.cemi.CEMILData#readAddInfo(java.io.
		...
		for (; remaining > 0; remaining -= 2) {
			...
			if (len > remaining || !checkAddInfoLength(type, len))
				throw new KNXFormatException(
						"invalid length " + len + " for additional info type 0x" + Integer.toHexString(type), len);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see tuwien.auto.calimero.cemi.CEMILData#readPayload(java.io.
	 * ByteArrayInputStream)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see tuwien.auto.calimero.cemi.CEMILData#writePayload(java.io.
