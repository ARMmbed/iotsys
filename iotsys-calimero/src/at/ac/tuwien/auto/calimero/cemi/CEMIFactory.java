...
public final class CEMIFactory {
	...
	/**
	 * Creates a new cEMI message out of the given <code>data</code> byte
	 * stream.
	 * <p>
	 * 
	 * @param data
	 *            byte stream containing a cEMI message frame structure
	 * @param offset
	 *            start offset of cEMI message in <code>data</code>
	 * @param length
	 *            length in bytes of the whole cEMI message in <code>data</code>
	 * @return the new created cEMI message
	 * @throws KNXFormatException
	 *             if no (valid) cEMI structure was found or unsupported cEMI
	 *             message code
	 */
	...
	/**
	 * Creates a new cEMI message with information provided by
	 * <code>original</code>, and adjusts it to match the supplied
	 * <code>msgCode</code> and <code>data</code>.
	 * <p>
	 * The message code has to correspond to the type of cEMI frame supplied
	 * with <code>original</code>. The byte length of data has to fit the cEMI
	 * frame type supplied with <code>original</code>.<br>
	 * The <code>data</code> argument varies according to the supplied message
	 * code. For L-Data frames, this is the tpdu, for busmonitor frames, this is
	 * the raw frame, for device management frames, this is the data part or
	 * error information.
	 * 
	 * @param msgCode
	 *            the message code for the new cEMI frame
	 * @param data
	 *            the data for the frame
	 * @param original
	 *            the original frame providing all necessary information for the
	 *            new frame
	 * @return the new cEMI frame adjusted with message code and data
	 * @throws KNXFormatException
	 *             if cEMI message code is unsupported or frame creation failed
	 */
	public static CEMI create(int msgCode, byte[] data, CEMI original) throws KNXFormatException {
		...
		case CEMIDevMgmt.MC_RESET_IND: {
			...
			return new CEMIDevMgmt(msgCode, f.getObjectType(), f.getObjectInstance(), f.getPID(), f.getStartIndex(),
					f.getElementCount(), data);
		}
		case CEMIBusMon.MC_BUSMON_IND:
	 ...
	 * <code>original</code>, and adjusts source and destination address to
	 * match the supplied addresses.
	 * <p>
	 * 
	 * @param src
	 *            the new KNX source address for the message, use
	 *            <code>null</code> to use original address
	 * @param dst
	 *            the new KNX destination address for the message, use
	 *            <code>null</code> to use original address
	 * @param original
	 *            the original frame providing all missing information for the
	 *            adjusted message
	 * @param extended
	 *            <code>true</code> to always created an extended frame,
	 *            <code>false</code> to create type according to
	 *            <code>original</code>
	 * @return the new cEMI message adjusted with KNX addresses
	 */
	public static CEMI create(IndividualAddress src, KNXAddress dst, CEMILData original, boolean extended) {
	...
	public static CEMI createFromEMI(byte[] frame) throws KNXFormatException {
		...
		if (mc == CEMIBusMon.MC_BUSMON_IND) {
			return new CEMIBusMon(frame[1] & 0xff, (frame[2] & 0xff) << 8 | frame[3] & 0xff, false,
					truncate(frame, 4, frame.length - 4));
		}
		...
		final KNXAddress a = (frame[6] & 0x80) != 0 ? (KNXAddress) new GroupAddress(dst) : new IndividualAddress(dst);
	 ...
	 * <p>
	 * Only for cEMI frames which are <b>not</b> immutable a copy is created,
	 * for all other frames <code>original</code> is returned.
	 * 
	 * @param original
	 *            the frame to copy
	 * @return the <code>original</code> frame if immutable, a copy of it
	 *         otherwise
	 */
	...
	private static CEMI create(int msgCode, IndividualAddress src, KNXAddress dst, byte[] data, CEMILData original,
			boolean ext) {
		...
		if (original instanceof CEMILDataEx) {
			...
			final CEMILDataEx copy = new CEMILDataEx(mc, s, d, content, f.getPriority(), f.isRepetition(),
		...
		if (ext)
			return new CEMILDataEx(mc, s, d, content, original.getPriority(), original.isRepetition(),
					original.getHopCount());
		return new CEMILData(mc, s, d, content, original.getPriority(), original.isRepetition(),
				original.getHopCount());
	}
