...
public class HostInfo implements DNSStatefulObject {
	...
	private DNSRecord.Address getDNS4AddressRecord(boolean unique, int ttl) {
		if ((this.getInetAddress() instanceof Inet4Address) || ((this.getInetAddress() instanceof Inet6Address)
	...
	private DNSRecord.Pointer getDNS4ReverseAddressRecord(boolean unique, int ttl) {
		...
		if ((this.getInetAddress() instanceof Inet6Address)
			...
			String address = (rawAddress[12] & 0xff) + "." + (rawAddress[13] & 0xff) + "." + (rawAddress[14] & 0xff)
					+ "." + (rawAddress[15] & 0xff);
			return new DNSRecord.Pointer(address + ".in-addr.arpa.", DNSRecordClass.CLASS_IN, unique, ttl,
					this.getName());
		}
		return null;
	}
	...
	private DNSRecord.Pointer getDNS6ReverseAddressRecord(boolean unique, int ttl) {
		if (this.getInetAddress() instanceof Inet6Address) {
			return new DNSRecord.Pointer(this.getInetAddress().getHostAddress() + ".ip6.arpa.", DNSRecordClass.CLASS_IN,
