...
public abstract class DNSEntry {
	...
	DNSEntry(String name, DNSRecordType type, DNSRecordClass recordClass, boolean unique) {
		...
		/// Changing from type to subtype to distinguish the key for both main
		/// and sub-typed services
		String subType = _qualifiedNameMap.get(Fields.Subtype).toLowerCase();
		_type = (subType.length() > 0 ? "_" + subType + "._sub." : "")
	...
	public boolean isSameEntry(DNSEntry entry) {
		return this.getKey().equals(entry.getKey()) && this.getRecordType().equals(entry.getRecordType())
				&& ((DNSRecordClass.CLASS_ANY == entry.getRecordClass())
						|| this.getRecordClass().equals(entry.getRecordClass()));
	}
