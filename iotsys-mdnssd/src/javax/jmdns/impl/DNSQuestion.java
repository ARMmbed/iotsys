...
public class DNSQuestion extends DNSEntry {
	...
	protected void addAnswersForServiceInfo(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers, ServiceInfoImpl info) {
		if ((info != null) && info.isAnnounced() && this.getName().contains("obix")) {
			logger.fine("this.getName(): " + this.getName() + " info.getQualifiedName(): " + info.getQualifiedName()
					+ " info.getType(): " + info.getTypeWithSubtype());
			if (this.getName().equalsIgnoreCase(info.getQualifiedName())
				...
				// addtnAnswers.addAll(info.addtnAnswers(DNSRecordClass.UNIQUE,
				// DNSConstants.DNS_TTL, jmDNSImpl.getLocalHost()));
			}
			logger.fine(jmDNSImpl.getName() + " DNSQuestion(" + this.getName() + ").addAnswersForServiceInfo(): info: "
					+ info + "\n" + answers);
		}
	}
