...
public class Responder extends DNSTask {
	...
	public void start(Timer timer) {
		// According to draft-cheshire-dnsext-multicastdns.txt chapter "7
		// Responding":
		...
		//
		// According to draft-cheshire-dnsext-multicastdns.txt chapter "6.2
		...
		int delay = (iAmTheOnlyOne
				&& !_in.isTruncated())
						? 0
						: DNSConstants.RESPONSE_MIN_WAIT_INTERVAL
								+ JmDNSImpl.getRandom()
										.nextInt(DNSConstants.RESPONSE_MAX_WAIT_INTERVAL
			...
			try {
				...
				if (!answers.isEmpty()) {
					...
					for (DNSRecord answer : answers) {
						if (answer != null) {
							// if
							// (answer.getRecordType().equals(DNSRecordType.TYPE_PTR))
