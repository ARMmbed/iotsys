...
public class TypeResolver extends DNSResolverTask {
	...
	protected DNSOutgoing addAnswers(DNSOutgoing out) throws IOException {
		...
		for (String type : this.getDns().getServiceTypes().keySet()) {
			...
			newOut = this.addAnswer(newOut, new DNSRecord.Pointer("_services._dns-sd._udp.local.",
					DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE, DNSConstants.DNS_TTL, typeEntry.getType()),
					now);
		}
		return newOut;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see javax.jmdns.impl.tasks.Resolver#addQuestions(javax.jmdns.impl.
	 * DNSOutgoing)
	 */
	@Override
