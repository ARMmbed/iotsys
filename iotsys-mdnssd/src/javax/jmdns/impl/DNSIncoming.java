...
public final class DNSIncoming extends DNSMessage {
	...
	public DNSIncoming clone() {
		DNSIncoming in = new DNSIncoming(this.getFlags(), this.getId(), this.isMulticast(), this._packet,
				this._receivedTime);
	...
	private DNSRecord readAnswer(InetAddress source) {
		...
		if ((recordClass == DNSRecordClass.CLASS_UNKNOWN) && (type != DNSRecordType.TYPE_OPT)) {
			logger.log(Level.SEVERE,
					"Could not find record class. domain: " + domain + " type: " + type + "\n" + this.print(true));
		}
		boolean unique = recordClass.isUnique(recordClassIndex);
		...
		case TYPE_PTR:
			...
			if (service.length() > 0) {
				...
				logger.log(Level.WARNING, "PTR record of class: " + recordClass
						+ ", there was a problem reading the service name of the answer for domain:" + domain);
			}
			break;
		case TYPE_TXT:
		...
		case TYPE_SRV:
			...
			String target = "";
			// This is a hack to handle a bug in the BonjourConformanceTest
			// It is sending out target strings that don't follow the "domain
			// name" format.
				...
				while (_messageInputStream.available() > 0) {
					...
					case Owner:
						...
						try {
							...
							logger.warning(
									"Malformed OPT answer. Option code: Owner data: " + this._hexString(optiondata));
						}
						if (logger.isLoggable(Level.FINE)) {
							logger.fine("Unhandled Owner OPT version: " + ownerVersion + " sequence: " + ownerSequence
					...
					case UL:
						if (logger.isLoggable(Level.FINE)) {
							logger.log(Level.FINE, "There was an OPT answer. Option code: " + optionCode + " data: "
