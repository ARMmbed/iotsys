...
public interface DNSStatefulObject {
			...
			if (this._task == task) {
				...
				try {
					if (this._task == task) {
						...
						logger.warning("Trying to advance state whhen not the owner. owner: " + this._task
								+ " perpetrator: " + task);
					}
				} finally {
					this.unlock();
	 ...
	 * @param task
	 *            associated task
	 * @return <code>true</code if the state was changed by this thread, <code>false</code>
	 *         otherwise.
