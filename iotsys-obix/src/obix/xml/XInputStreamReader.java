...
/**
 * XInputStreamRead is used to read a XML byte stream into a stream of unicode
 * characters. Mapping the byte stream into a charset encoding is implemented
 * according XML 1.0 Appendix F - Autodetection of Character Encodings.
 * XInputStream also automatically handles reading a PKZIP zipped XML. Character
 * encodings supported:
 * <ul>
 ...
 * <li>PKZIP containing any other format with 0x504b0304 byte order mark
 * (assumes text is first zip entry)</li>
...
public class XInputStreamReader extends Reader {
	...
	private int readUtf8(int c0) throws IOException {
		...
		case 15:
			...
			/*
			 * I think this is valid, but Java doesn't seem to output characters
			 * this high - so cap things below this c1 = in.read(); c2 =
			 * in.read(); c3 = in.read(); if (((c1 & 0xC0) != 0x80) || ((c2 &
			 * 0xC0) != 0x80) || ((c3 & 0xC0) != 0x80)) throw new
			 * UTFDataFormatException(); return ((c0 & 0x07) << 18) | ((c1 &
			 * 0x3F) << 12) | ((c2 & 0x3F) << 6) | ((c3 & 0x3F) << 0);
			 */
			throw new UTFDataFormatException();
		default:
			throw new UTFDataFormatException();
	...
	static final String[] ENCODINGS = { "UTF-8", // 0
			"UTF-16BE", // 1
			"UTF-16LE", // 2
	};
