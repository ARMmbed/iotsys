...
public class Obj implements IObj, Subject, AlarmSource, Cloneable {
	...
	public String getTranslation(String language, TranslationAttribute attribute) {
		...
		if (language == null || !translations.containsKey(attribute)
				|| (!translations.get(attribute).containsKey(language)
						&& !translations.get(attribute).containsKey(DEFAULT_LANGUAGE))) {
			switch (attribute) {
			case display:
	...
	/**
	 * @return <code>true</code> if this Obj is currently in an alarm condition,
