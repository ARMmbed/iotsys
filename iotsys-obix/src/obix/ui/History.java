...
public class History {
	...
	/**
	 * Touch an ord (which moves it to the top of the most recent list).
	 */
	 ...
	 */
	/*
	 * public void load() { try { File file = new File(home, "recents.xml"); if
	 * (!file.exists()) return;
	 * 
	 * XElem root = XParser.make(file).parse(); XElem[] xEntries =
	 * root.elems("entry"); for(int i=0; i<xEntries.length; ++i) { XElem x =
	 * xEntries[i];
	 * 
	 * String name = x.get("name"); String uri = x.get("uri");
	 * 
	 * recent.add(new Entry(name, uri, icon)); } } catch(Exception e) {
	 * e.printStackTrace(); } }
	 * 
	 * /** Save to file.
	 */
	/*
	 * public void save() { try { File file = new File(home, "recent.xml");
	 * XWriter out = new XWriter(file); out.prolog(); out.w("<recent>\n");
	 * for(int i=0; i<recent.size(); ++i) { Entry e = (Entry)recent.get(i);
	 * out.w("  <entry") .attr(" name", e.name) .attr(" uri", e.uri); if (e.icon
	 * != null) out.attr(" icon", e.icon); out.w("/>\n"); }
