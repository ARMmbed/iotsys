...
public class ViewRegistry {
	...
	public static View[] viewsFor(Shell shell, UiSession.Response resp) throws Exception {
		ArrayList<View> acc = new ArrayList<>();
	...
	public static View[] viewsForContract(Shell shell, UiSession.Response resp, Uri contract) throws Exception {
		...
		if (uri.equals("obix:WatchService"))
			return viewsForWatchService(shell, resp);
		if (uri.equals("obix:AlarmSubject"))
			return viewsForAlarmSubject(shell, resp);
		return null;
	}

	/**
	 * Get the views for the WatchService. There is always a WatchServiceView
	 * used to make and manage watches. Plus there is a WatchView for each
