...
/**
 * ObjSheet provides a property sheet like interface to view and edit an obix
 * Obj tree.
 *
 * @author Brian Frank
 * @creation 13 Sept 05
 ...
 */
public class ObjSheet extends Editor {
	...
	/**
	 * Prompt the user with a dialog to edit the specified target Obj. Return
	 * null if dialog canceled.
	 */
	public static Obj prompt(Shell shell, String title, UiSession session, Obj target) throws Exception {
		...
		if (pane.getValue() == null || pane.getValue().equals(new Integer(JOptionPane.CANCEL_OPTION)))
			return null;
		sheet.save(target);
		return target;
	}
	...
	/**
	 * This version of save is used to walk the object tree and call
	 * SaveVisitor.save(Obj) for each Val node which has been changed.
	 */
	...
	class ObjLabel extends JLabel {
		...
		public void processMouseEvent(MouseEvent e) {
			...
			if (shell == null || target.getHref() == null)
				return;
	...
	/*
	 * public static void main(String[] args) throws Exception { Obj obj = new
	 * Obj("foo"); obj.add(new Str("bar", "hello")); obj.add(new Str("cool",
	 * "world")); Obj x = new Obj("x"); obj.add(x); x.add(new Bool("bf",
	 * false)); x.add(new Bool("bt", true)); x.add(new Int("i", 102)); x.add(new
	 * Real("r", 123.456)); x.add(new Enum("e", "foo")); x.add(new Uri("uri",
	 * "http://www.tridium.com/")); Obj result = prompt((JComponent)null,
	 * "Test", null, obj); if (result == null) { System.out.println("CANCEL"); }
	 * else { ObixEncoder.dump(result); } System.exit(0); }
	 */
