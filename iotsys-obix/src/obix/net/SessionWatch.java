...
/**
 * SessionWatch manages a client side watch a session. See
 * ObixSession.makeWatch().
 *
 * @author Brian Frank
 * @creation 12 Sept 05
...
public class SessionWatch {
	...
	/**
	 * Get the time (millis since epoch) that the obj at the specified index was
	 * updated by a poll operation.
	 */
	...
	/**
	 * Add the specified uris to this watch and return the current values.
	 */
	...
	/**
	 * Get number of milliseconds for the lease time. This time is specified the
	 * server.
	 */
	...
	/**
	 * Set number of milliseconds for the lease time. This method will
	 * synchronously attempt to change the lease time (blocks until network
	 * request finishes). Return the actual lease time given to us by the
	 * server.
	 */
	...
	/**
	 * Get number of milliseconds between each poll. This value is initialized
	 * specified in ObixSession.makeWatch(), but can be changed via
	 * setPollPeriod.
	 */
	...
	/**
	 * Set number of milliseconds between each poll. The initial pollPeriod is
	 * specified during ObixSession.makeWatch(). This method will check that the
	 * lease is safely larger than the pollPeriod - if not this method will
	 * synchronously attempt to change the lease time (blocks until network
	 * request finishes). It is possible that the server will not honor the
	 * required lease time, in which case we will automatically shorten our
	 * pollPeriod accordingly. This method doesn't take effect until the next
	 * poll cycle - although you can force an immediate poll using the
	 * pollChanges() method. Return actual pollPeriod in use.
	 */
	...
	/**
	 * Synchronously poll the server for just changes which have occurred since
	 * the last poll.
	 */
	...
	/**
	 * Synchronously poll the server for a completely fresh update.
	 */
