...
/**
 * ObixSession models a client side logic connection to an obix server using the
 * HTTP REST protocol. Only HTTP basic authentication is supported.
 *
 * @author Brian Frank
 * @creation 12 Sept 05
...
public class ObixSession {
	...
	/**
	 * Construct a session where uri specifies the lobby URL of a obix server.
	 * For example lobby typically looks like "http://foo/obix". Username and
	 * password provide the credentials needed to access the server.
	 */
	 ...
	 * @param password
	 * @param timeout
	 *            the timeout used for both connectTimeout and readTimeout
	 *            (default is 0).
	 */
	...
	/**
	 * Get the authority uri of this session, which is basically the domain
	 * name: "http://host/". All requests to this session must be within the
	...
	/**
	 * Return if the specified uri can be processed by this session. A session
	 * can only process uris within it's authority. For example a session for
	 * http://foo/ cannot be used to process requests for http://bar/. This
	 * method will return true for any relative uri. This method is really a
	...
	/**
	 * Return the lobby object. This object is first read on session open().
	 */
	public Obj getLobby() {
		if (lobby == null)
	...
	/**
	 * Attempt to read the server's about object. If successful return it,
	...
	/**
	 * Read the obix document as an Obj instance using the specified uri
	 * relative to the base address of the sesssion.
	 */
	...
	/**
	 * Write the specified obj back to the server using the obj's href. Return a
	 * new object containing the server's result.
	 */
	...
	/**
	 * Invoke the op at the given href with the given input parameter and return
	...
	/**
	 * Make a BatchIn request which can be used to store up a bunch of read,
	 * write, invoke requests then sent over the network in one shot by calling
	 * BatchIn.commit().
	 */
	...
	/**
	 * Perform an HTTP get request on the specified uri relative to the base
	 * address of the session and return an input stream to read the resource.
	 */
	...
	/**
	 * Get a watch by name or return null if no watched mapped to the specified
	 * name.
	 */
	...
	/**
	 * Make a new empty Watch for this session. Name is unique key used as
	 * programmer's moniker for debugging (used by UI code too). The pollPeriod
	 * is used to specify how fast the watch should poll for changes. This value
	 * will automatically be used to request a longer lease time if needed. If
	 * listener is not null, automatically add it to receive changed callbacks.
	 */
	...
	/**
	 * Return a fully configured HttpURLConnection for the given URI and HTTP
	 * method.
	 */
	protected HttpURLConnection setupHttpURLConnection(Uri uri, String method) throws Exception {
	...
	/**
	 * Provides a simple interactive command line tool which reads obix objects
	 * using an ObixSession and dumps the result to stdout.
	 */
	...
	static void dumpContent(ObixSession session, Uri uri) throws Exception {
	...
	static Obj dumpObj(ObixSession session, Uri uri) throws Exception {
