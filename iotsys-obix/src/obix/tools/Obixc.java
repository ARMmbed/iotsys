...
/**
 * Obixc compiles obix prototypes into Java source files. Each well known obj
 * (has href) gets mapped into a interface and a class. The interface defines
 * the sub-object accessor methods. The class implements the accessors and
 * provides a static init() to add defaults to an Obj's hash map.
 *
 * @author Brian Frank
 * @creation 27 Apr 05
...
public class Obixc {
	...
	/**
	 * Write the Java source for an interface to model the specified type. The
	 * interface contains an accessor method for each child object. If the child
	 * object has meta-data like facets or it's own contract then we also
	 * generate a "contract field" with the meta-data defined a string contract
	 * with the actual XML.
	 */
	...
	/**
	 * We only include contract fields for children which aren't simple object
	 * types with just a name - examples include when the child object specifies
	 * facets, a default val attribute, or has it's own contract list. The way
	 * we test is to compare the encoding for the actual object against what the
	 * default constructor does. This matches what ObixAssembler does when
	...
	public void writeContractInit() throws Exception {
		...
		for (int i = 0; i < types.length; ++i) {
			Type type = types[i];
			w("    ContractRegistry.put(\"").w(type.href.get()).w("\", \"").w(packageName).w(".").w(type.name)
					.w("\");\n");
		}
		w("  }\n");
		w("\n");
