...
/**
 * ObixAssembler uses the Java bytecode assembly library to dynamically generate
 * classes which extend from one of the core base types (Obj, Bool, Real, etc)
 * and also implement a list of interfaces to model a contract.
 *
 * @author Brian Frank
 * @creation 24 May 06
 * @version $Revision$ $Date: 9/9/2003 3:39:17 PM$
 */
public class ObixAssembler extends Assembler implements OpCodes {

	/**
	 * Compile a class which extends from superClass, and implements all the
	...
	/**
	 * Process a single method which should represent a single sub-object from a
	 * contract.
	 */
	private void compileSubObject(Code ctor, Method m) {
		...
		// {
		// public static final String dimensionContract = "<obj name='dimension'
		// is='obix:Dimension'/>";
	...
	/*
	 * public static interface A extends IObj {} public static interface B
	 * extends IObj { Str foo(); Bool enabled(); }
	 * 
	 * public static void main(String[] args) throws Exception { Class cls =
	 * compile(obix.Real.class, new Class[] { A.class, B.class });
	 * System.out.println("  class   " + cls); System.out.println("  extends " +
	 * cls.getSuperclass().getName()); Class[] interfaces = cls.getInterfaces();
	 * for (int i=0; i<interfaces.length; ++i)
	 * System.out.println("  implements " + interfaces[i].getName());
	 * 
	 * Real obj = (Real)cls.newInstance(); obj.dump(); B b = (B)obj;
	 * 
	 * System.out.println(" enabled: " + b.enabled());
	 * System.out.println(" foo:     " + b.foo()); b.foo().setStr("rocking!");
