...
public class SimpleHVACvalveActuatorImplKnx extends SimpleHVACvalveActuatorImpl {
	...
	public SimpleHVACvalveActuatorImplKnx(KNXConnector knxConnector, GroupAddress status,
			final GroupAddress valuePosition) {
		super();
		this.status = status;
			...
			knxConnector.addWatchDog(valuePosition, new KNXWatchDog() {
				...
				public void notifyWatchDog(byte[] apdu) {
					try {
						...
						// System.out.println("+++++DEBUB APDU:"+
						// getHexString(apdu));
						// System.out.println("getDate:
						// "+getHexString(x.getData()));
	...
	public void refreshObject() {
		// if(status != null){
		// int value =
		// knxConnector.readInt(status,ProcessCommunicator.UNSCALED);//
		// SCALING);
