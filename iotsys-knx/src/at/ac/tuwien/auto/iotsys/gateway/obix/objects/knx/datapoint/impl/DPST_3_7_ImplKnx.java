...
public class DPST_3_7_ImplKnx extends DPST_3_7_Impl {
	...
	public DPST_3_7_ImplKnx(KNXConnector connector, GroupAddress groupAddress, String name, String displayName,
			String display) {
	...
	private Obj doRun(Obj in, boolean control) {
		try {
			if (in instanceof ParameterDimming) {
				...
				if (value.get() > 0)
					stepCode = (int) Math
							.round(((Math.log((float) ParameterDimming.MAX_VALUE / value.get()) / Math.log(2)) + 1));
