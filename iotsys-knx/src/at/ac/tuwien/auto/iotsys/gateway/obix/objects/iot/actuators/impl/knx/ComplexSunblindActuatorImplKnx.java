...
public class ComplexSunblindActuatorImplKnx extends ComplexSunblindActuatorImpl {
	...
	public ComplexSunblindActuatorImplKnx(KNXConnector knxConnector, GroupAddress statusPositionBlind,
			GroupAddress statusPositionSlat, final GroupAddress positonBlindValue,
			final GroupAddress positonSlatValue) {
		super();
		...
		if ((this.statusPositionBlindGA == null) || (this.statusPositionSlatGA == null)) {
			...
			log.info(
					"For ComplexSunblindActuatorImplKnx the Parameter statusPositionBlind and statusPositionSlat have to be set.");
			...
			knxConnector.addWatchDog(statusPositionBlind, new KNXWatchDog() {
				...
				public void notifyWatchDog(byte[] apdu) {
					try {
						DPTXlator8BitUnsigned x = new DPTXlator8BitUnsigned(DPTXlator8BitUnsigned.DPT_SCALING);// DPT_VALUE_1_UCOUNT);//
																												// DPT_SCALING);
						...
						if (x.getValueUnsigned(
								2) != (short) ComplexSunblindActuatorImplKnx.this.positonBlindValue.get()) {

							ComplexSunblindActuatorImplKnx.this.positonBlindValue.set(x.getValueUnsigned(2));
							// SimpleHVACvalveActuatorImplKnx.this.value.set(x.getValueUnsigned(2));
							// // getValueUnscaled());
							log.info("Updated to value: "
									+ (short) ComplexSunblindActuatorImplKnx.this.positonBlindValue.get());
						}
			...
			knxConnector.addWatchDog(statusPositionSlat, new KNXWatchDog() {
				...
				public void notifyWatchDog(byte[] apdu) {
					try {
						...
						if (x.getValueUnsigned(
								2) != (short) ComplexSunblindActuatorImplKnx.this.positonSlatValue.get()) {

							ComplexSunblindActuatorImplKnx.this.positonSlatValue.set(x.getValueUnsigned(2)); // getValueUnscaled());
							log.info("Updated to value: "
									+ (short) ComplexSunblindActuatorImplKnx.this.positonSlatValue.get());
						}
	...
	public void writeObject(Obj input) {
		...
		if (input instanceof ComplexSunblindActuator) {
			...
			// log.info("+++++Old
			// Value"+(short)ComplexSunblindActuatorImplKnx.this.positonSlatValue.get());
			...
			if (input.getHref() == null) {
				...
				if (input.getInvokedHref() != null && input.getInvokedHref().length() > 0) {
					String resourceUriPath = input.getInvokedHref()
							.substring(input.getInvokedHref().lastIndexOf('/') + 1);

					if (ComplexSunblindActuator.POSITION_BLIND_HIGH_CONTRACT_HREF.equals(resourceUriPath)) {
						knxConnector.write(positonBlindValueGA, (int) this.positonBlindValue().get(),
						...
						knxConnector.write(positonSlatValueGA, (int) this.positonSlatValue().get(),
								ProcessCommunicator.SCALING);
		...
		// super.writeObject(input);
		//
		// knxConnector.write(positonBlindValueGA,
		// (int)this.positonBlindValue().get(), ProcessCommunicator.SCALING);
