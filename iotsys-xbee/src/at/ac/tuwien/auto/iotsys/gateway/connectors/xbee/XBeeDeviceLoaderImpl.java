...
public class XBeeDeviceLoaderImpl implements DeviceLoader {
	...
	private final static Logger log = Logger.getLogger(XBeeDeviceLoaderImpl.class.getName());
	...
	public ArrayList<Connector> initDevices(ObjectBroker objectBroker) {
		...
		if (connectorsFromDb.size() <= 0) {
			Object xbeeConnectors = devicesConfig.getProperty("xbee.connector.name");
		...
		for (int connector = 0; connector < connectorsSize; connector++) {
			HierarchicalConfiguration subConfig = devicesConfig.configurationAt("xbee.connector(" + connector + ")");
			...
			if (enabled) {
				try {
					...
					XBeeConnector xBeeConnector = new XBeeConnector(serialPort, 9600);
					...
					log.info(numberOfDevices + " XBee devices found in configuration for connector " + connectorName);
								...
								for (int k = 0; k < declaredConstructors.length; k++) {
									if (declaredConstructors[k].getParameterTypes().length == args.length) { // constructor
										...
										try {
											...
											Obj xBeeDevice = (Obj) declaredConstructors[k].newInstance(args);
											...
											if (historyEnabled != null && historyEnabled) {
												if (historyCount != null && historyCount != 0) {
													objectBroker.addHistoryToDatapoints(xBeeDevice, historyCount);
												} else {
													objectBroker.addHistoryToDatapoints(xBeeDevice);
												}
											}

											if (groupCommEnabled) {
												objectBroker.enableGroupComm(xBeeDevice);
											}

											if (refreshEnabled != null && refreshEnabled) {
												objectBroker.enableObjectRefresh(xBeeDevice);
											}
