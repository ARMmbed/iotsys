...
public class WaitingRoom {
	...
	public Key enterServer(InetSocketAddress peer, Network network, byte id) {
		// We set the server value in the key to false so that it matches with
	...
	public void notifyMember(InetSocketAddress peer, Network network, byte id, boolean isFromServer, APDU apdu)
		...
		// The member may not have gotten around to listening for a message yet,
		// so enter a retry loop to
		// make sure that this message gets to where it's supposed to go if
	...
	/**
	 * This class is used by network message controllers to manage the blocking
	 * of threads sending confirmed messages. The instance itself serves as a
	 * monitor upon which the sending thread can wait (with a timeout). When a
	 * response is received, the message controller can set it in here,
	 * automatically notifying the sending thread that the response is
	 * available.
	 * 
	 * @author mlohbihler
	 */
