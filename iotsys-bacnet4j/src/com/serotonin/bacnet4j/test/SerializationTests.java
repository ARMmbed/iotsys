...
public class SerializationTests {
	...
	private static void testEncodable(Encodable encodable) throws Exception {
		...
		if (BaseError.class.isAssignableFrom(encodable.getClass()))
			...
			deserialized = UnconfirmedRequestService
					.createUnconfirmedRequestService(((Service) encodable).getChoiceId(), queue);
		else {
	...
	private static final Encodable[] encodables = { new ReadPropertyAck(
			new ObjectIdentifier(ObjectType.eventEnrollment, 123), PropertyIdentifier.eventParameters, null,
			new com.serotonin.bacnet4j.type.eventParameter.FloatingLimit(new UnsignedInteger(234),
			...
			new ReadPropertyAck(new ObjectIdentifier(ObjectType.analogValue, 7), PropertyIdentifier.priorityArray, null,
					new PriorityArray()), //
