...
public class LoopDevice implements Runnable {
	...
	public LoopDevice(String broadcastAddress, int port) throws BACnetServiceException, IOException {
		...
		try {
			...
			// for valid property values with valid datatypes see
			// com.serotonin.bacnet4j.obj.ObjectProperties and ther
			...
			ai0.setProperty(PropertyIdentifier.objectName, new CharacterString("G1-RLT03-TM-01")); // this
																									// is
																									// a
																									// cryptic
			// encoded name of a
	...
	public void run() {
		try {
			...
			while (!isTerminate()) {
				...
				synchronized (this) {
					wait(1000); // 1 second or notified (faster exit then stupid
								// wait for 1 second)
				}
			}
