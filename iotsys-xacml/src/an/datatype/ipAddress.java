...
/**
 * IP address The "urn:oasis:names:tc:xacml:2.0:data-type:ipAddress" primitive
 * type represents an IPv4 or IPv6 network address, with optional mask and
 * optional port or port range. The syntax SHALL be:
 * 
 * ipAddress = address [ "/" mask ] [ ":" [ portrange ] ]
 * 
 * For an IPv4 address, the address and mask are formatted in accordance with
 * the syntax for a "host" in IETF RFC 2396 "Uniform Resource Identifiers (URI):
 * Generic Syntax", section 3.2. For an IPv6 address, the address and mask are
 * formatted in accordance with the syntax for an "ipv6reference" in IETF RFC
 * 2732 "Format for Literal IPv6 Addresses in URL's". (Note that an IPv6 address
 * or mask, in this syntax, is enclosed in literal "[" "]" brackets.)
 */
		...
		if (value != null && value.matches(IPADDRESS_PATTERN)) {
			...
			try {
				...
				if (addr.startsWith("[") && addr.endsWith("]")
						...
						if (msk != null) {
							...
							if (msksIPv6 != null) {
								...
								throw new InvalidIPAddressException(
										"The mask address doesn't match the format of ip address.");
							}
						}
					...
					// potential IPv4
					// FIXME - XACML 2.0 says the ipv4 address may be a host
					// name. I think should it a typo? if not, I
