...
public abstract class MatchFunctions {
	...
	public static AttributeValue x500NameMatch(EvaluationContext ctx, Object[] params) throws IndeterminateException {
		checkArguments(params, 2);
		AttributeValue pattern = (AttributeValue) params[0];
		AttributeValue toBeMatched = (AttributeValue) params[1];
	...
	public static AttributeValue rfc822NameMatch(EvaluationContext ctx, Object[] params) throws IndeterminateException {
		checkArguments(params, 2);
		AttributeValue pattern = (AttributeValue) params[0];
		AttributeValue toBeMatched = (AttributeValue) params[1];
		...
		if (atIndex > 0) {
			...
			return rfc822Source.getLocalPart().equals(local) && rfc822Source.getDomain().equalsIgnoreCase(domain) ? TRUE
					: FALSE;
