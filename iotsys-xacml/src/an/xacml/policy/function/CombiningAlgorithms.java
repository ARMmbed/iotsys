...
public abstract class CombiningAlgorithms {
	private static void checkArguments(Object[] params, int expectedNumber) throws IndeterminateException {
		...
		if (expectedNumber > 0 && params.length != expectedNumber) {
			throw new IndeterminateException(
					"Expected " + expectedNumber + " parameters, but got " + params.length + ".");
		}
	}
	...
	/**
	 * Decision denyOverridesRuleCombiningAlgorithm(Rule rule[]) { Boolean
	 * atLeastOneError = false; Boolean potentialDeny = false; Boolean
	 * atLeastOnePermit = false; for (i = 0; i < lengthOf(rules); i ++) {
	 * Decision decision = evaluate(rule[i]); if (decision == Deny) { return
	 * Deny; } if (decision == Permit) { atLeastOnePermit = true; continue; } if
	 * (decision == NotApplicable) { continue; } if (decision == Indeterminate)
	 * { atLeastOneError = true;
	 * 
	 * if (effect(rule[i]) == Deny) { potentialDeny = true; } continue; } } if
	 * (potentialDeny) { return Indeterminate; } if (atLeastOnePermit) { return
	 * Permit; } if (atLeastOneError) { return Indeterminate; } return
	 * NotApplicable; }
	 * 
	 * @param request
	 * @param rules
	 ...
	 * @throws IndeterminateException
	 */
	@XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:rule-combining-algorithm:deny-overrides",
			"urn:oasis:names:tc:xacml:1.1:rule-combining-algorithm:ordered-deny-overrides" })
	public static Result ruleDenyOverrides(EvaluationContext ctx, Object[] params) throws IndeterminateException {
	...
	/**
	 * Decision permitOverridesRuleCombiningAlgorithm(Rule rule[]) { Boolean
	 * atLeastOneError = false; Boolean potentialPermit = false; Boolean
	 * atLeastOneDeny = false; for (i=0 ; i < lengthOf(rule) ; i ++) { Decision
	 * decision = evaluate(rule[i]); if (decision == Deny) { atLeastOneDeny =
	 * true; continue; } if (decision == Permit) { return Permit; } if (decision
	 * == NotApplicable) { continue; } if (decision == Indeterminate) {
	 * atLeastOneError = true;
	 * 
	 * if (effect(rule[i]) == Permit) { potentialPermit = true; } continue; } }
	 * if (potentialPermit) { return Indeterminate; } if (atLeastOneDeny) {
	 * return Deny; } if (atLeastOneError) { return Indeterminate; } return
	 * NotApplicable; }
	 * 
	 * @param request
	 * @param rules
	 ...
	 * @throws IndeterminateException
	 */
	@XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:rule-combining-algorithm:permit-overrides",
			"urn:oasis:names:tc:xacml:1.1:rule-combining-algorithm:ordered-permit-overrides" })
	public static Result rulePermitOverrides(EvaluationContext ctx, Object[] params) throws IndeterminateException {
	...
	/**
	 * Decision firstApplicableEffectRuleCombiningAlgorithm(Rule rule[]) { for
	 * (i = 0 ; i < lengthOf(rule); i ++) { Decision decision =
	 * evaluate(rule[i]); if (decision == Deny) { return Deny; } if (decision ==
	 * Permit) { return Permit; } if (decision == NotApplicable) { continue; }
	 * if (decision == Indeterminate) { return Indeterminate; } } return
	 * NotApplicable; }
	 * 
	 * @param request
	 * @param rules
	...
	public static Result ruleFirstApplicable(EvaluationContext ctx, Object[] params) throws IndeterminateException {
		checkArguments(params, 3);
		Rule[] rules = (Rule[]) params[0];
		for (Rule rule : rules) {
			Result result = rule.evaluate(ctx);
	...
	/**
	 * Decision denyOverridesPolicyCombiningAlgorithm(Policy policy[]) { Boolean
	 * atLeastOnePermit = false; for(i=0 ; i < lengthOf(policy); i ++) {
	 * Decision decision = evaluate(policy[i]); if (decision == Deny) { return
	 * Deny; } if (decision == Permit) { atLeastOnePermit = true; continue; } if
	 * (decision == NotApplicable) { continue; } if (decision == Indeterminate)
	 * { return Deny; } } if (atLeastOnePermit) { return Permit; } return
	 * NotApplicable; }
	 * 
	 * @param request
	 * @param policies
	 ...
	 * @return
	 */
	@XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:policy-combining-algorithm:deny-overrides",
	 ...
	 * Boolean atLeastOneError = false; Boolean atLeastOneDeny = false; for (i =
	 * 0; i < lengthOf(policy); i ++) { Decision decision = evaluate(policy[i]);
	 * if (decision == Deny) { atLeastOneDeny = true; continue; } if (decision
	 * == Permit) { return Permit; } if (decision == NotApplicable) { continue;
	 * } if (decision == Indeterminate) { atLeastOneError = true; continue; } }
	 * if (atLeastOneDeny) { return Deny; } if (atLeastOneError) { return
	 * Indeterminate; } return NotApplicable; }
	 * 
	 * @param request
	 * @param policies
	 * @param cmbParams
	 ...
	 * @throws IndeterminateException
	 */
	@XACMLFunction({ "urn:oasis:names:tc:xacml:1.0:policy-combining-algorithm:permit-overrides",
	 ...
	 * for (i = 0; i < lengthOf(policy); i ++) { Decision decision =
	 * evaluate(policy[i]); if(decision == Deny) { return Deny; } if(decision ==
	 * Permit) { return Permit; } if (decision == NotApplicable) { continue; }
	 * if (decision == Indeterminate) { return Indeterminate; } } return
	 * NotApplicable; }
	 * 
	 ...
	 * { Boolean atLeastOne = false; Policy selectedPolicy = null;
	 * ApplicableResult appResult;
	 * 
	 * for (i = 0; i < lengthOf(policy); i ++) { appResult =
	 * isApplicable(policy[I]);
	 * 
	 * if (appResult == Indeterminate) { return Indeterminate; } if(appResult ==
	 * Applicable) { if (atLeastOne) { return Indeterminate; } else { atLeastOne
	 * = true; selectedPolicy = policy[i]; } } if (appResult == NotApplicable) {
	 * continue; } } if (atLeastOne) { return evaluate(selectedPolicy); } else {
