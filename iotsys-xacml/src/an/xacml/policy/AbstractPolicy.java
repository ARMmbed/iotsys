...
public abstract class AbstractPolicy extends DefaultCacheable implements Evaluatable, Matchable {
	/**
	 * The property intends to provider namespace mapping for XPath evaluation.
	 * The value actually is a map that includes all attributes in Policy or
	 * PolicySet element and with "xmlns:" in attribute name. For example, if a
	 * Policy element has following attributes, some of them have "xmlns:" in
	 * name, and some of them don't have,
	 *
	 ...
	 * ... </Policy>
	 * 
	 * Then the namespace mapping should include following attributes name and
	 * value mappings,
	 * 
	...
	protected static void appendPolicyObligationsToResult(Result result, Obligations obls, EvaluationContext ctx,
			boolean supportInnerExp) {
		...
		if (supportInnerExp) {
			// Trying to evaluate all AttributeAssignments on the cloned
			// Obligations.
			try {
				appender.evaluateAllChildAttributeAssigments(ctx);
			}
			// If there is error, we don't process it, just append the
			// AttributeAssignments, and the error should be
			// processed by PEP.
