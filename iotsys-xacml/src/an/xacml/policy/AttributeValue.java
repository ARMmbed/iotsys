...
public class AttributeValue extends DefaultXACMLElement implements Expression, AdditionalNamespaceMappingEntry {
	...
	// the attribute value can include child expression, we should evaluate it
	// and return the result.
	protected Expression childExp;

	// Since the content of attribute value may introduce additional namespaces.
	// We need add these namespaces to the top
	// level element, then they can be retrieved by program that requires those
	// namespaces, such as a attribute selector.
	private Map<String, String> additionalNSMappings;

	/**
	 * Construct from a typed value. The value should be come from context's
	 * Attribute. The context's Attribute element is in charge of parse the
	 * value and convert it to a typed value.
	 * 
	 * @param value
	...
	protected AttributeValue(URI xmlType, Object value) throws XMLDataTypeMappingException {
		...
		if (!expectedType.isAssignableFrom(actualType)) {
			throw new XMLDataTypeMappingException("The attribute value's type '" + actualType.getName()
	...
	/**
	 * Construct from a String value. The value should be come from underlying
	 * data store. The data store adapter is in charge of parse the value in
	 * data store. This class is in charge of convert it to a typed value.
	 * 
	 * @param value
	 * @throws XMLDataTypeMappingException
	 */
	...
	/**
	 * Just return self if there is no child expression, or return the child
	...
	public Object evaluate(EvaluationContext ctx) throws IndeterminateException {
		...
		if (supportInnerExpression() && this.childExp != null) {
			...
			if (result.getClass().isArray()) {
				...
				// If multiple AttributeValue instances are returned, we will
			...
			// If the child expression's data type does not match the parents'
			// one, we will throw an exception.
			if (!finalResult.getDataType().equals(this.dataType)) {
				throw new IndeterminateException(
						"The child expression's data type '" + finalResult.getDataType()
								+ "' doesn't match the parents' data type '" + dataType + "'.",
						Constants.STATUS_SYNTAXERROR);
			}
			return finalResult;
		}
	...
	public Object getValue() throws IndeterminateException {
		...
		if (supportInnerExpression() && this.childExp != null) {
			// We don't need return the value of AttributeValue because the
			// caller will get the most inner expression's
			// AttributeValue which has no child expression, caller can directly
			// get value from it.
	...
	/**
	 * If this element has additional namespace mappings that required by
	 * attribtue selector, we need add them to the top level Policy or
	 * PolicySet.
	 */
	...
	protected boolean supportInnerExpression() {
		...
		// If we are in response, or we are not in PDP, then PEP should in
		// charge of evaluate it.
		return false;
	}
}