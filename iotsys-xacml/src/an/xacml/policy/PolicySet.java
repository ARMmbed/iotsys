...
public class PolicySet extends AbstractPolicy {
	...
	/**
	 * Merge all policies to an array, and this array will be used for
	 * evaluation.
	 * 
	...
	public Result evaluate(EvaluationContext ctx) throws IndeterminateException {
		try {
			...
			if (target.match(ctx)) {
				// We don't call the method from PolicySet's constructor,
				// because referenced policies may not be loaded while
				...
				// Get rule-combine-alg function from function registry, and
				// then pass rules, combinerParams and
				...
				Result policyResult = (Result) policyCombAlg.invoke(ctx, new Object[] { allPolicies, combinerParameters,
						policyCombinerParameters, policySetCombinerParameters });
				// Retrieve the corresponding Obligations by Effect in
				// EvaluationResult, and set it to EvaluationResult.
				if ((policyResult.getDecision() == Permit || policyResult.getDecision() == Deny)
						&& obligations != null) {
