...
public abstract class DefaultMatch extends DefaultXACMLElement implements Matchable {
	/**
	 * It's a functionId, which referenced function must return a boolean
	 * result.
	 */
	...
	/**
	 * The method get a built-in function and invoke it by passing attribute
	 * value and result of an expression (a designator or a attribute selector)
	 * as its parameters, and then return a boolean result indicate if the match
	 * is successful or failed. If any error occurs, an IndeterminateException
	 * will be thrown (XACML 2.0 requires).
	 */
	public boolean match(EvaluationContext ctx) throws IndeterminateException {
		try {
			...
			if (param2 != null) {
				// The <Match> element requires to match each of attributes
				// returned from designator or selector,
