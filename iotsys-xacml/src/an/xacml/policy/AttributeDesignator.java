...
public class AttributeDesignator extends DefaultXACMLElement implements Expression {
	...
	// Indicate if the designate attribute must be present, if it is true and
	// the attribute is missing, an
	...
	/**
	 * Return the selected attribute values from request. The returned type
	...
	protected AttributeValue[] getAttributesFromRequest(EvaluationContext ctx) throws IndeterminateException {
		try {
			...
			if ((result == null || result.length == 0) && supportMustBePresent() && mustBePresent) {
				...
				// Return an IndeterminateException with an array of
				// MissingAttributeDetail. This array object will be
	...
	/**
	 * Determine if current PDP supports "mustBePresent" feature. If we don't
	 * support such feature, there is no exception throws if the expected
	 * attribute doesn't exist in request and "mustBePresent" is set to "true".
