...
public class IdReference extends DefaultXACMLElement {
			...
			for (PolicyResolver resolver : resolvers) {
				if (resolver.isPolicySupported(id)) {
					...
					if (resolved != null && validateResolvedPolicy(resolved)) {
						...
						logger.warn("The policy resolved by " + resolver.getClass().getSimpleName() + (resolved == null
	...
	private boolean validateResolvedPolicy(AbstractPolicy policy) {
		...
		if ((version != null && !version.match(ver)) || (version != null && !earliestVersion.match(ver))
	...
	/**
	 * XACML 2.0 doesn't define the situation that policy's version doesn't
	 * match the IdReference's one, I just throw an PolicySyntaxException for
	 * this situation.
	 * 
	 * @return
	 * @throws PolicySyntaxException
