...
/**
 * The default implementation of DataStore, if "an.xacml.engine.DataStore" is
 * not configured, this defult DataStore will be used.
 */
public class XMLFileDataStore implements DataStore {
	...
	/**
	 * Load all policies from a specific path, return an Iterator of
	 * DataAdapter, caller may call DataAdapter.getEngineElement method to get
	 * the corresponding engine element which is going to be evaluated.
	 * 
	...
	public DataAdapter[] load() throws DataAdapterException {
		try {
			...
			if (dir.isDirectory()) {
				...
				// Retrieve the schema file from given path or classpath, and
				// verify if it exists.
				...
				for (int i = 0; i < policyFiles.length; i++) {
					try {
						...
						// check if there are duplicated policy IDs in different
						// files
						URI policyId = getPolicyOrPolicySetId(root);
						if (policyFilesByID.get(policyId) != null) {
							throw new DataAdapterException("The policy loaded from '" + policyFiles[i] + "' with ID<"
									+ policyId + "> already exists.");
						}

						// This is used to build up a URI->Policy File index.
						// The index will be used in load(URI[])
						// method.
						...
						if (logger.isDebugEnabled()) {
							try {
								// Create a new dataAdapter, do not use the
								// loaded one, since it holds the original
								// XML element. We should use the one data
								// adapter generated.
				...
				if (actualPolicyNum > 0) {
					...
					System.out.println(
							"\n" + actualPolicyNum + " policies loaded. " + "Time elapsed " + (end - begin) / 1000
									+ " second. " + "Memory used " + (memEnd - memBegin) / 1024 / 1024 + " MB.");
					if (error || warn) {
	...
	/**
	 * Get policy id if parsed Element is a Policy, or get policySet id if
	 * parsed Element is a PolicySet. There should not have multiple policies or
	 * policySets in a single file.
	 * 
	 * @param element
	 * @return
	...
	private URI getPolicyOrPolicySetId(Element element) throws URISyntaxException, PolicySyntaxException {
		...
		if (id == null || "".equals(id)) {
			// This should not be happened since we have checked schema while
			// parsing.
			throw new PolicySyntaxException(
					"The element '" + element.getLocalName() + "' doesn't include PolicyId or PolicySetId attribute.");
		}
		return new URI(id);
	}
	...
	public void save() throws DataAdapterException {
		...
		try {
			...
			// Retrieve the schema file from given path or classpath, and verify
			// if it exists.
			...
			for (int i = 0; i < allPolicies.length; i++) {
				...
				// If no existing file, this means the policy is new added, we
				// create a new file for it.
				if (policyFile == null) {
					policyFile = new File(POLICY_FILE_PREFIX + policy.getId() + POLICY_FILE_SUFFIX);
				}
				// Dump the xml element to file. Note, we don't need clean up
				// the directory.
	...
	/**
	 * Only update some of policies in data store. The new added policies are
	 * included in "toBeUpdated". "toBeDeleted" can only include IDs of
	 * policies.
	 */
