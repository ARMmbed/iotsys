...
public class XACMLParser {
	...
	public static AbstractPolicy parsePolicy(InputStream in) throws Exception {
		// Retrieve the schema file from given path or classpath, and verify if
		// it exists.
	...
	public static void dumpPolicy(AbstractPolicy policy, OutputStream out) throws Exception {
		...
		// Validate the element in case there are policies were created
		// dynamically and not validated.
	...
	public static Request parseRequest(InputStream in) throws Exception {
		...
		// parse request file
		// System.out.println(XACMLParser.class.getSimpleName() +
		// ".parseRequest(): " + defaultSchema);
	...
	public static void dumpRequest(Request request, OutputStream out) throws Exception {
		...
		// Validate the element in case there are requests were created
		// dynamically and not validated.
		Element element = (Element) da.getDataStoreObject();
		validateElement(element, defaultSchema);
		// Dump the xml element to out.
	...
	public static void dumpResponse(Response response, OutputStream out) throws Exception {
		...
		// Validate the element in case there are responses were created
		// dynamically and not validated.
		Element element = (Element) da.getDataStoreObject();
		validateElement(element, defaultSchema);
		// Dump the xml element to out.
	...
	static DataAdapter createPolicyDataAdapterFromXMLElement(Element elem) throws Exception {
		...
		// initialize the root XML element from DOM element. The root element
		// will iterate each child element and
		// construct them.
		DataAdapter da = (DataAdapter) constructor.newInstance(elem);
		// Iterate each engine XACML element and set parent element. We have to
		// populate parent element here because
		// engine element was not ready while constructing XMLElement.
		// FIXME should this operation be moved to Policy or PolicySet's
		// constructor?
	...
	static DataAdapter createContextDataAdapterFromXMLElement(Element elem) throws Exception {
		// Get the element's type
		// System.out.println("createContextDataAdapterFromXMLElement:
		// ClassName: " + elem.getClass().getName());
		// System.out.println("createContextDataAdapterFromXMLElement:
		// LocalName: " + elem.getLocalName());
		// System.out.println("createContextDataAdapterFromXMLElement:
		// SchemaTypeInfo: " + elem.getSchemaTypeInfo().toString());
