...
public class FileAdapterStatusDetail extends AbstractFileAdapterContextElement {
	 ...
	 * <xs:complexType name="StatusDetailType"> <xs:sequence>
	...
	public FileAdapterStatusDetail(Element elem) throws Exception {
		...
		// First determine if this element is MissingAttributeDetail, otherwise,
		// we'll convert it to plain text
		Object detail = null;
		...
		if (any != null && any.toString().length() > 0) {
			if (any.getClass().isArray()) {
				...
				for (int i = 0; i < size; i++) {
					...
					// First determine if this is a MissingAttributeDetail, if
					// no, we'll convert it to plain text
					Element elem = null;
					if (component instanceof MissingAttributeDetail) {
						elem = (Element) new FileAdapterMissingAttributeDetail((MissingAttributeDetail) component)
								.getDataStoreObject();
					} else {
						elem = getDefaultDocument().createElementNS(CONTEXT_NAMESPACE,
				...
				// First determine if this is a MissingAttributeDetail, if no,
				// we'll convert it to plain text
				Element elem = null;
				if (any instanceof MissingAttributeDetail) {
					elem = (Element) new FileAdapterMissingAttributeDetail((MissingAttributeDetail) any)
							.getDataStoreObject();
				} else {
					elem = getDefaultDocument().createElementNS(CONTEXT_NAMESPACE, any.getClass().getSimpleName());
