...
public class FileAdapterMissingAttributeDetail extends AbstractFileAdapterContextElement {
	/**
	 * <xs:element name="MissingAttributeDetail" type=
	 * "xacml-context:MissingAttributeDetailType"/>
	 * <xs:complexType name="MissingAttributeDetailType"> <xs:sequence>
	 * <xs:element ref="xacml-context:AttributeValue" minOccurs="0" maxOccurs=
	...
	public FileAdapterMissingAttributeDetail(Element elem) throws Exception {
		...
		/*
		 * NodeList attrValues =
		 * elem.getElementsByTagName(ELEMENT_MISSINGATTRIBUTEDETAIL); int len =
		 * attrValues.getLength(); String[] values = new String[len]; for (int i
		 * = 0; i < len; i ++) { values[i] =
		 * XMLParserWrapper.getNodeContentAsText(attrValues.item(i)); }
		 */
