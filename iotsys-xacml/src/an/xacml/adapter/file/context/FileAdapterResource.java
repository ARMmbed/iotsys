...
public class FileAdapterResource extends FileAdapterTargetElement {
	 ...
	 * <xs:element ref="xacml-context:Attribute" minOccurs="0" maxOccurs=
	...
	public FileAdapterResource(Element elem) throws PolicySyntaxException, XMLGeneralException {
		...
		engineElem = new Resource(chRC == null ? null : (ResourceContent) ((DataAdapter) chRC).getEngineElement(),
				attrs);
	...
	public FileAdapterResource(XACMLElement engineElem) throws Exception {
		...
		if (res.getResourceContent() != null) {
			xmlElement.appendChild(
					(Element) new FileAdapterResourceContent(res.getResourceContent()).getDataStoreObject());
		}
