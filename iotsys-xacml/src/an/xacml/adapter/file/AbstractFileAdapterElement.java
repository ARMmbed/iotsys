...
public abstract class AbstractFileAdapterElement extends AbstractXMLElement {
	...
	public static final String[] CAN_BE_MERGED = { COMBINER_PARAMETERS, RULE_COMBINER_PARAMETERS,
			POLICY_COMBINER_PARAMETERS, POLICY_SET_COMBINER_PARAMETERS };
			...
			if (element != null) {
				...
				while (node != null) {
					...
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						...
						// If this child is a registered element(may be simple
						// or complex type), we lookup a registered
						...
						// If this child isn't registered. We considered this is
						// a simple type or derived from a simple
						// type, we will wrap it to a TextXMLElement.
						else {
							NodeList grandChildren = el.getChildNodes();
							// If this element has only one child node, and it
							// is a text node, we consider this is a
							// simple type or derived from a simple type, we
							// direct use TextXMLElement.
							if (grandChildren.getLength() == 1
								...
								// The text node may has attributes, we extract
								// and add them here.
								((TextXMLElement) xmlEl)
										.addAttributes(((TextXMLElement) xmlEl).extractXMLAttributes(el));
							}
							// We don't know why it has more than one children
							// or it has no child, so we just construct
							// a TextXMLElement from the whole content of
							// element.
							else {
	...
	/**
	 * Append a child element to current element, and set current element as
	 * parent of child element.
	 */
	public synchronized void addChildElement(XMLElement child) {
		// Check if this child element is needed to be merged with existing same
		// element.
		if (isMergeableElement(child)) {
			for (int i = 0; i < childElements.length; i++) {
				XMLElement item = childElements[i];
				// If there is existing element, we need to merge the child with
				// existing element.
				if (canBeMerged(child, item)) {
					item.addChildElements(child.getChildElements());
					// We don't need iterate to next element, because the
					// existing child elements should not include
	...
	public synchronized void addChildElements(XMLElement[] children) {
		// Since we need perform check on each child element, we have to call
		// addChildElement circularly
	...
	private static boolean canBeMerged(XMLElement a, XMLElement b) {
		if (a.getElementName().equals(b.getElementName()) && a.getNamespaceURI().equals(b.getNamespaceURI())) {
			...
			if (elemName.equals(COMBINER_PARAMETERS)) {
				...
				return ((RuleCombinerParameters) ((DataAdapter) a).getEngineElement()).getRuleId()
						.equals(((RuleCombinerParameters) ((DataAdapter) b).getEngineElement()).getRuleId());
			} else if (elemName.equals(POLICY_COMBINER_PARAMETERS)) {
				return ((PolicyCombinerParameters) ((DataAdapter) a).getEngineElement()).getPolicyId()
						.equals(((PolicyCombinerParameters) ((DataAdapter) b).getEngineElement()).getPolicyId());
			} else if (elemName.equals(POLICY_SET_COMBINER_PARAMETERS)) {
				return ((PolicySetCombinerParameters) ((DataAdapter) a).getEngineElement()).getPolicySetId()
						.equals(((PolicySetCombinerParameters) ((DataAdapter) b).getEngineElement()).getPolicySetId());
			}
		}
		return false;
	}
	...
	/*
	 * We have to use the same document to create both policy and context
	 * element, because if we use different documents create elements under the
	 * same root element, there will be validate errors. This should not be an
	 * issue, because the elements should be validated using the external schema
	 * after they have created.
	 */
	...
	protected Class<?> getElementClassFromSystem(String elemType) {
		...
		try {
			...
			// If it still not an XML primitive type, we consider it is a
			// TextXMLElement type.
