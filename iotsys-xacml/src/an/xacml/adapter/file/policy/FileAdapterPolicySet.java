...
public class FileAdapterPolicySet extends AbstractFileAdapterPolicyElement {
	 ...
	 * <xs:complexType name="PolicySetType"> <xs:sequence>
	 * <xs:element ref="xacml:Description" minOccurs="0"/>
	 * <xs:element ref="xacml:PolicySetDefaults" minOccurs="0"/>
	 * <xs:element ref="xacml:Target"/>
	 * <xs:choice minOccurs="0" maxOccurs="unbounded">
	 * <xs:element ref="xacml:PolicySet"/> <xs:element ref="xacml:Policy"/>
	 * <xs:element ref="xacml:PolicySetIdReference"/>
	 * <xs:element ref="xacml:PolicyIdReference"/>
	 * <xs:element ref="xacml:CombinerParameters"/>
	 * <xs:element ref="xacml:PolicyCombinerParameters"/>
	 * <xs:element ref="xacml:PolicySetCombinerParameters"/> </xs:choice>
	 * <xs:element ref="xacml:Obligations" minOccurs="0"/> </xs:sequence>
	...
	public FileAdapterPolicySet(Element elem) throws PolicySyntaxException, XMLGeneralException {
		...
		// Since PolicyCombinerParameters inherit from CombinerParameters, the
		// method will return multiple items. We
		// have to get it by element name. And since the CombinerParameters have
		// been merged in initialize, we will get
		// only one element.
		...
		// The policy XMLSchema allows multiple PolicyCombinerParameters
		// elements, but XACML standard doesn't allow that.
		...
		// The policy XMLSchema allows multiple Obligations elements, but XACML
	...
	public FileAdapterPolicySet(XACMLElement engineElem) throws Exception {
		...
		if (policySet.getCombinerParameters() != null) {
			xmlElement.appendChild((Element) new FileAdapterCombinerParameters(policySet.getCombinerParameters())
					.getDataStoreObject());
		}
		if (policySet.getPolicyCombinerParameters() != null) {
			xmlElement.appendChild(
					(Element) new FileAdapterPolicyCombinerParameters(policySet.getPolicyCombinerParameters())
							.getDataStoreObject());
		}
		if (policySet.getPolicySetCombinerParameters() != null) {
			xmlElement.appendChild(
					(Element) new FileAdapterPolicySetCombinerParameters(policySet.getPolicySetCombinerParameters())
							.getDataStoreObject());
		}
		if (policySet.getObligations() != null) {
			xmlElement
					.appendChild((Element) new FileAdapterObligations(policySet.getObligations()).getDataStoreObject());
		}
