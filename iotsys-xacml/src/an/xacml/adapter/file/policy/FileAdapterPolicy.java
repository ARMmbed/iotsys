...
public class FileAdapterPolicy extends AbstractFileAdapterPolicyElement {
	 ...
	 * <xs:complexType name="PolicyType"> <xs:sequence>
	 * <xs:element ref="xacml:Description" minOccurs="0"/>
	 * <xs:element ref="xacml:PolicyDefaults" minOccurs="0"/>
	 * <xs:element ref="xacml:CombinerParameters" minOccurs="0"/>
	 * <xs:element ref="xacml:Target"/> <xs:choice maxOccurs="unbounded">
	 * <xs:element ref="xacml:CombinerParameters" minOccurs="0"/>
	...
	public FileAdapterPolicy(Element elem) throws PolicySyntaxException, XMLGeneralException {
		...
		// Since RuleCombinerParameters inherit from CombinerParameters, the
		// method will return 2 items. We have to get
		// it by element name. And since the CombinerParameters have been merged
		// in initialize, we will get only one
		// element.
		...
		// The policy XMLSchema allows multiple RuleCombinerParameters elements,
		// but XACML standard doesn't allow that.
	...
	public FileAdapterPolicy(XACMLElement engineElem) throws Exception {
		...
		if (policy.getCombinerParameters() != null) {
			xmlElement.appendChild(
					(Element) new FileAdapterCombinerParameters(policy.getCombinerParameters()).getDataStoreObject());
		}
		...
		if (policy.getRuleCombinerParameters() != null) {
			xmlElement.appendChild((Element) new FileAdapterRuleCombinerParameters(policy.getRuleCombinerParameters())
					.getDataStoreObject());
		}
