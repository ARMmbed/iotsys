...
/**
 * The default implementation of ContextHandler, if
 * "an.xacml.engine.ContextHandler" is not configured, this defult
 * ContextHandler will be used instead.
 */
public class DefaultContextHandler implements ContextHandler {
	...
	public Object handle(Object reqCtx) {
		...
		try {
			...
			if (evalResult == null) {
				// pass all potential matched policies to PDP for final
				// decision. This method also process
				// NotApplicable situation. We don't need perform match
				// operation on each potential matched
				// policy because we will perform "evaluate" operation on them
				// in subsequent call.
				evalResult = pdp.getMultiPoliciesDecision(new EvaluationContext(req),
						cacheMgr.getPoliciesByRequest(req));
			}
			...
			if (!(e instanceof IndeterminateException)) {
				e = new IndeterminateException("Server encounter errors while handling the request", e,
						Constants.STATUS_SERVERERROR);
			}
			evalResult = new Result((IndeterminateException) e);
		}

		// ctxFactory won't be null because we have initialize it while starting
		// PDP
