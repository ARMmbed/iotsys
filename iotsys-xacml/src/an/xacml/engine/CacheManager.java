 ...
 * 
 * There are 2 types of caches that used in PDP. One is policy cache, which is
 * used to cache all policies. Policies are loaded from a underlying data store,
 * such as policy files under a directory of local file system. Policies are
 * added to a hash map cache using Target as index, so that when a request is
 * coming, we can use request's Target as key to get the corresponding matched
 * polices, then to evaluate policies.
 * 
 * The other is decision cache, which is used to cache the decisions. Decisions
 * are got from policy's evaluation result, and will be added to decision cache
 * using request as index. So that when a request is coming, we first retrieve
 * corresponding decision from decision cache using the request, if there is
 * same request has evaluated before, then we can get a decision from cache, and
 * we don't need retrieve policies and evaluate them. Except using request as
 * index, decision cache also has another index, which is policy. It is used to
 * remove corresponding decision from decision cache when a policy is updated.
 * 
 */
public class CacheManager {
	...
	/**
	 * Each PDP has its own CacheManager, pass a PDP object will get
	 * corresponding CacheManager instance.
	 * 
	 ...
	 */
	protected void addEvalResult(Request request, Result result) throws CacheSizeExceedCapacityException {
		if (enableResultCache) {
	...
	/**
	 * This method is only used to load policies from underlying data store, it
	 * should not used to update policies from PAP. To update policies, use
	 * updatePolicies instead.
	 * 
	 * @param policy
	 * @throws CacheSizeExceedCapacityException
	...
	/**
	 * When PAP is sending updated policies, this method will be called to
	 * update those policies in cache. The method will update policies in policy
	 * cache by specific IDs, and invalidate all evaluation results.
	 * 
	 * @param policies
	 * @throws CacheSizeExceedCapacityException
				...
				try {
					...
					if (toBeDeleted != null && toBeDeleted.length > 0) {
						...
						for (int i = 0; i < toBeDeleted.length; i++) {
							...
							// Construct a dumy policy that only include ID,
							// since to remove an entry from cache only
							// requires the id.
	...
	private DataAdapter createDataAdapterFromEngineElement(XACMLElement engineElem) throws Exception {
		...
		// All file adapters should have a constructor with a parameter that
		// type is "XACMLElement"
