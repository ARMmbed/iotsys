...
public class PolicyCache extends Cache {
	...
	public void add(AbstractPolicy policy) throws BuiltInFunctionNotFoundException, CacheSizeExceedCapacityException {
				...
				for (TargetElement each : allElems) {
					...
					if (attrs != null && attrs.length > 0) {
						for (Attribute attr : attrs) {
							// Retrieve policies from cache using attribute
							// index
							...
							// If current cache support value index, we add
	...
	private List<AttributeIndex> extractAttributeIndexesFromPolicy(AbstractPolicy policy)
		...
		// loop in Subjects, Actions, Resources and Environments to get child
		// Subject[], Action[], Resource[] and
		// Environment[].
		for (DisjunctiveMatch dMatch : targetElems) {
			Matchable[] cMatches = dMatch.getMatchables();
			if (cMatches != null && cMatches.length > 0) {
				// loop in each Subject, Action, Resource and Environment to get
				// child SubjectMatch, ActionMatch,
				...
				for (int i = 0; i < cMatches.length; i++) {
					...
					if (defaultMatches != null && defaultMatches.length > 0) {
						// loop in each SubjectMatch, ActionMatch, ResourceMatch
						// and EnvironmentMatch to get child
						...
						for (int j = 0; j < defaultMatches.length; j++) {
							...
							if (expression instanceof AttributeDesignator) {
								...
								// If current PDP support mustBePresent and the
								// attribute designator also required
								// the attribute must be present, we don't add
								...
								if (!pdp.supportMustBePresent() || !designator.isAttributeMustBePresent()) {
									...
									/*
									 * Considering a policy which requires
									 * "attr1" and "attr2" attributes. If there
									 * is an AttributeRetriever in system
									 * supports all of these 2 attributes,
									 * regardless the coming request ships those
									 * attributes or not, this policy should be
									 * evaluated against the request anyway,
									 * because it can get those attributes from
									 * AttributeRetriever. Also, this policy
									 * should be evaluated against any request
									 * since it doesn't require attributes from
									 * request, it can get them from
									 * AttributeRetriever. If we add this policy
									 * to the cache use attribute keys, it will
									 * never be retrieved by request that
									 * doesn't ship the corresponding
									 * attributes.
									 *
									 * That is to say, if an attribute is
									 * supported by AttributeRetriever, we
									 * should not require the coming request to
									 * have this attribute. So we will treat
									 * above policy as if it has an empty
									 * target, and it will be added to a the
									 * empty target area ( line 61).
									 */
									// check if there are attribute retrievers
									// support current attribute, if there is,
	...
	private boolean isMatchFunctionEquivalentType(DefaultMatch match) throws BuiltInFunctionNotFoundException {
		try {
			...
			// code won't run to here because we have already get the
