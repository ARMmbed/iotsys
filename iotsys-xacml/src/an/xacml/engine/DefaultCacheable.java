...
/**
 * Provides a default implementation of Cacheable, which will be inherited by
 * PolicySet and Policy. All of the method should use read-write lock to do
 * synchronizition.
 */
public abstract class DefaultCacheable extends DefaultXACMLElement implements Cacheable {
	...
	private Cache owner;
	// This property enables a Cacheable scope expire interval, it will
	// interwork with the one configured
	// on the owner Cache. Subclass should assign value to it if need the
	// feature.
	protected long selfExpireInterval;

	/**
	 * Add current Cacheable to a Cache by given key. This method is designed to
	 * invoke from its subclass, such as Policy, PolicySet or Decision.
	 * 
	 * @param key
	 * @param cache
	...
	public boolean isExpired() {
		...
		synchronized (expLock) {
			if (expired) {
				...
				// If finalExpireInterval is less than 0, then the Cacheable
				// will never expired unless force set it
				// to expired.
