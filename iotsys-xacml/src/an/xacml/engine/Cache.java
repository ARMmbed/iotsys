...
public abstract class Cache {
	...
	/**
	 * Should use read-write lock to do synchronization. This method is designed
	 * for called from Cacheable's implementation.
	 * 
	 * The cache supports add multiple cacheable objects with same key, in this
	 * case, it will create a list to keep all of these same key objects, then
	 * put the list to cache. This feature is to support multiple policies match
	...
	/**
	 * Get the cacheable, and then check if it is expired, if expired, remove it
	 * from cache, and then return null.
	 * 
	 * @return
	 */
	...
	/**
	 * Get all cacheables from the cache. Check each cacheable if there is
	 * expired. It should take a certain long time because it will cleanup all
	 * expirations. Did not find who need to call it, so make it protected.
	 * 
	 * @return
	 */
	...
	/**
	 * Add a Cacheable to internal cache map. No lock operation, for internal
	 * use only.
	 * 
	 * @param key
	 * @param o
	...
	protected void addInternal(Object key, Cacheable o) {
		...
		if (origin == null) {
			...
			for (int i = 0; i < origin.size(); i++) {
				...
				// equals should compare Cacheable's id(policy id, policySet id
				// or decision's serials no.)
				if (o.equals(item)) {
	...
	/**
	 * Clean up expirations in cache. The method will fork a new thread to do
	 * clean up. it is invoked internally.
	 */
		...
		synchronized (running) {
			...
			Thread cleanThread = new Thread() {
				...
				private void removeSomeExpirations() {
					int loop = 0;
					// We only remove some of expired cacheables for 1 calling.
					// This is intend to let those
					// add operations in. Otherwise, the add operation will wait
					// for a long time untill all
	...
	/**
	 * If the cacheLock is null, we don't do synchronization. The sub-classes
