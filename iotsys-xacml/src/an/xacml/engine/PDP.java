...
/**
 * This class creates a PDP, which will load and cache all XACML policies. There
 * may have more than one PDP in a JVM. Each PDP should have a unique domain
 * name, domain name is the PDP's identity.
 */
public class PDP extends AbstractMonitorableAndControllable {
	...
	protected void loadPolicies() throws DataAdapterException, CacheSizeExceedCapacityException, PolicySyntaxException,
	...
	public synchronized static PDP getInstance(ConfigElement config) throws PDPInitializeException {
	...
	/**
	 * This method read a configuration entry to determine which decision will
	 * be used when multiple PolicySets match the same request. It will be
	 * called by ContextHandler during evaluating policies.
	 */
	public Result getMultiPoliciesDecision(EvaluationContext ctx, AbstractPolicy[] policies)
		...
		if (policies == null || policies.length == 0) {
			...
			// retrieve the function, and then evaluate the policies in the
			// function.
			try {
				...
				// The IndeterminateException throws from evaluation of
				// policies.
				if (e instanceof IndeterminateException) {
					intEx = (IndeterminateException) e;
				}
				// Other exceptions, such as BuiltInFunctionNotFoundException,
				// we treat it as processing error.
				else {
					intEx = new IndeterminateException("Server encounter errors while handling the request", e,
	...
	/**
	 * This method creates a ContextHandler, and then pass a request context to
	 * it for process.
	 * 
	...
	public void reloadPolicies() throws DataAdapterException, CacheSizeExceedCapacityException, PolicySyntaxException,
			BuiltInFunctionNotFoundException {
	...
	public synchronized void start() throws OperationFailedException {
		...
		if (current.equals(STATUS_RUN_INITIALIZED)) {
			try {
				startPDP();
				System.out
						.println("PDP '" + getDomain() + "' has been started. Version <" + version.getVersion() + ">");
			} catch (Exception e) {
