...
public class Request extends DefaultXACMLElement {
	...
	/**
	 * At least one Subject should be in the array. If there are same
	 * "subject-category" in different Subject elements, then all these Subject
	 * elements should be merged to a single Subject element. The merging should
	 * be happened before the RequestElement is created. So following subjects
	 * are the results after done such merging.
	 */
	...
	/**
	 * The request root element of request element, which is provided for
	 * AttributeSelector. If request isn't constructed from XML element, we will
	 * build up one from all target elements.
	 */
	...
	public Element getRootNode() throws Exception {
		if (requestElement == null) {
			// If the request isn't constructed from XML element, we will
			// generate an XML element for it. So that it
			// doesn't depend on the file based request to evaluate
			// AttributeSelector.
	...
	/**
	 * Generate the hashCode from all fields. If a RequestElement equals to the
	 * other, then their hashCode should also be equal. If 2 RequestElement are
	 * equal, their Index should also be equal.
	 */
	public int hashCode() {
		return hashCode;
	}
