...
public class Status extends DefaultXACMLElement {
	...
	public Status(IndeterminateException e) {
		...
		// log the status detail since it may not be included in the final
		// response.
		...
		// Only missing attribute will return a StatusDetail object. This is for
		// indicating which attribute is missing.
	...
	/**
	 * A copy constructor which is used to create a new Status instance from an
	 * existing one.
	 * 
	 * @param status
	 */
	public Status(Status status) {
		// Since the StatusCode & StatusDetail are all constant classes, we can
		// directly assign them to the new instances.
		// If we need make them mutable in the future, we should also modify
		// them to use a copy constructor.
	...
	public boolean equals(Object o) {
		...
		if (o != null && o.getClass() == this.getClass()) {
			...
			// We don't compare the message if one has no message.
					(message == null || other.message == null) ? true
							: compareObject(message, other.message) && compareObject(detail, other.detail);
		}
		return false;
	}
