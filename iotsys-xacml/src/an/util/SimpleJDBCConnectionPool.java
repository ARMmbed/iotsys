...
/**
 * We provide a simple Connection Pool for use. This pool supports a fixed size,
...
public class SimpleJDBCConnectionPool {
	...
	public SimpleJDBCConnectionPool(String driver, String connectionURL, String dbUser, byte[] password, int size,
			int timeout) throws ConnectionPoolException {
		try {
	...
	private void initialize(String connectionURL, String dbUser, byte[] password, int size)
		...
		for (int i = 0; i < size; i++) {
			...
			pool.add((PooledConnection) Proxy.newProxyInstance(PooledConnection.class.getClassLoader(),
	...
	/**
	 * The main method of the pool. Returns an available connection. If we can't
	 * get an available connection within "timeout", we will throw an exception.
	 * 
	 * @return
	...
	/**
	 * If the given connection is broken, we'll try to repair it. If we can't
	 * repair it, we will throw an exception.
	 * 
	 * @param conn
	 */
	...
	/**
	 * If we can't get an available connection in a single loop, we will fork
	 * another loop, until we get it or timeout. This method is only used to be
	 * called from itself.
	 * 
	 * @param timeElapsed
	...
	private Connection getConnection(long timeElapsed) throws ConnectionPoolException {
		...
		// If we haven't get yet, we will re-loop to get it. Before we re-loop,
		// we will take a rest for 100ms or 10% of
		// the "timeout".
		try {
	...
	class PooledConnectionHandler implements InvocationHandler {
		...
		/**
		 * We will cache a real Connection to the handler. While method call
		 * coming to the handler, we can make the call to the real Connection.
		 * 
		 * @param conn
		 */
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			if (method.getName().equals("close")) {
				// close method should be called by Connection user after they
				// accomplish their tasks. We should
