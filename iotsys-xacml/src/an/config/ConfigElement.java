...
public class ConfigElement extends AbstractXMLElement {
	 ...
	 * 
	 * In this constructor, we don't copy parent element. The parent will be set
	 * while it is copying as a child element. If we clone a ConfigElement
	 * object from a leaf node (not from root), the cloned leaf element will
	 * become the root element of the cloned tree.
	 * 
	 * @param config
	 *            The ConfigElement object that need to be cloned.
	 */
	...
	/**
	 * Override the method from supper class, only use ConfigElement itself to
	 * initialize configuration, because only attributes are useful in
	 * configuration.
	 * 
	 * @param element
	 * @return
	...
	/**
	 * FIXME This method should be moved to AbstractXMLElement class after the
	...
	protected boolean isDerivedFrom(String ns, String typeName) {
		if (typeName != null) {
			...
			return thisType.isDerivedFrom((ns == null ? thisType.getTypeNamespace() : ns), typeName,
	...
	public synchronized XMLElement[] getXMLElementsByName(String name) {
		...
		// We explicitly return a ConfigElement[] typed result to avoid cast
		// issues.
