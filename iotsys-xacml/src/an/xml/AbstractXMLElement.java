...
/**
 * FIXME We are going to generalize all XACML file adapters and configuration
 * elements, then we can use a single class to represent all elements. So from
 * now on, we will add some methods that conform this design, such as
 * getXMLElementsByType(String elementType). And first we will try refactor
 * configuration elements, and then we will start XACML file adapters.
 */
public abstract class AbstractXMLElement implements XMLElement {
	/**
	 * The child XML elements should be assigned value in subclasses'
	 * constructor.
	 */
	...
	/**
	 * The namespace of this XML element, will be used if save this object to
	 * XML file.
	 */
	protected String elementNamespaceURI;
	/**
	 * Each element should has a name. Multiple elements with same type may
	 * occur in a same parent element, but they should have different names.
	 */
	...
	/**
	 * Append an attribute to current element, and set current element as owner
	 * of the attribute.
	 */
	...
	protected XMLAttribute[] extractXMLAttributes(Element element) throws XMLGeneralException {
		...
		if (element != null) {
			...
			for (int i = 0; i < attrs.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ATTRIBUTE_NODE && node instanceof Attr && (attrNS == null ? true
	...
	public synchronized XMLElement[] getXMLElementsByType(Class<?> type) {
		...
		// Must use the passed-in class type as toArray's parameter. If use "new
		// XMLElement[0]" directly, the result
		// will be convert to a XMLElement[] type and with XMLElement subclass'
		// object in it. It cannot be down
		// cast to an array such as PDPConfigElement[], since the Array type
		// could not be down cast.
	...
	public synchronized XMLElement[] getXMLElementsByName(String name) {
		...
		// Since the elements' type of the array may different, so we return a
		// generic XMLElement[] type.
