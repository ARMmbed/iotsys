...
public final class XMLDataTypeRegistry {
	...
	static {
		// Map primitive types to their wrapper types. We only map those types
		// that are used by XACML.
		...
		// Below data types are defined in the core standard of XACML 2.0,
		// however, an errata published at 29 Jan, 2008
		// deleted them and then added them as XACML specific data types. But to
		// keep compatibility with the core
		// standard (especially for conformance test), we will support both
		// these types and the corrected data types.
	...
	public static Object getTypedValue(Class<?> type, String value) throws XMLDataTypeMappingException {
		...
		if (type.isArray()) {
			...
			// convert time/dateTime and other data type to XMLGregorianCalendar
			// or Duration class.
			try {
		...
		throw new XMLDataTypeMappingException(
				"The string value \"" + value + "\" could not be converted to class \"" + type.getName() + "\"");
	}
