...
/**
 * A default logger we provided. With this logger, you don't need employ any of
 * 3rd logging tool -- either log4j nor Java logging tool.
 */
public class DefaultLogger implements Logger {
	...
	protected DefaultLogger(ConfigElement config, String tag) throws IOException {
		...
		synchronized (outputMap) {
			// Each log file has a file writer, we don't create new ones for the
			// same file. We kept the writer
			// in a map.
			...
			if (out == null) {
				...
				// If there is error occurs, we print the log message to System
	...
	/**
	 * If no tag is set, we get the call stack (Class name and line number) as
	 * tag.
	 */
	public String getTag() {
		if (tag == null) {
			...
			for (int i = 1; i < traces.length; i++) {
				// Get the first "non-Logger" class name as calling stack's
				// class name.
	...
	/**
	 * A cleanup method that close all output stream, this method will be called
	 * before system is shutting down.
	 */
	...
	protected String formatMessage(int level, Object message) {
		...
		msgFormatter.format(STR_FORMAT_PATTERM, "[" + getDateTime() + "]", "<" + LEVELS[level] + ">",
				"<" + getLocalHostName() + ">", "<" + getCurrentThread() + ">", "<" + getTag() + ">", message);
	...
	protected void writeLog(int level, Object message, Throwable t) {
		if (checkLevel(level)) {
			try {
				...
				System.out.println(
						"Error occurs when writing log: " + e.getMessage() + ". Will print log to system out.");
	...
	protected boolean checkLevel(int level) {
		...
		// If current class configured specific log level, we use it instead of
		// the global level.
	...
	/**
	 * If current class has configured specific log level, this method is to get
	 * the level.
	 * 
	 * @return
	 */
	...
	/**
	 * Compare current level to configured level, if current level is allowed to
	 * be logged, then return true, otherwise, return false.
	 * 
	 * @param level
	 * @param configLevel
	 * @param single
	...
	protected boolean isCurrentLevelMatchingConfig(int level, int configLevel, boolean single) {
		// If single level mode is enabled, we only log the exact level's
		// message.
		// If single level mode is disabled, we log all levels that lower than
		// current level.
	...
	/**
	 * Check if the current log file size has exceeded the limitation. If
	 * exceeded the limitation, rename current log file to add a ".n" trailing,
	 * and then create a new one to write log to.
	 * 
	 * @param msg
	 * @throws IOException
	 */
	protected void checkRollover(String msg, Throwable t) throws IOException {
		if (rolloverSize > 0) {
			...
			// We also need compute the exception's stack traces which we will
			// write to log.
			...
			// Compute the size include current message which need to be written
			// to current log file.
	...
	protected String getCurrentClassName() {
		...
		StackTraceElement[] traces = ex.getStackTrace();
		for (int i = 1; i < traces.length; i++) {
			// Get the first "non-Logger" class name as calling stack's class
			// name.
