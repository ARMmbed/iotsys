...
/**
 * The LogFactory is used to obtain a Logger. User should first pass an
 * LogConfigElement to initialize the factory, then call getLogger() with
 * default tag or call getLogger(String) to provide a custom tag for the logger.
 * 
 * Currently, we provided 2 types of logger, one is an internal logger, the
 * other is a log4j logger. To provide such a log system is intend to avoid
 * libraries conflict in employer system.
 * 
 * Users may provide their custom implementation of interface Logger, they
 * should provide the implementation's class name(configuration attribute
 * an.log.Logger's value) with such a constructor:
 * CustomLogger(CustomLoggerConfigElement config, String tag). Users also need
 * provide a corresponding CustomLoggerConfigElement class and configurations in
 * config file.
 */
public class LogFactory {
	...
	/**
	 * Initialize the factory. Currently, the factory doesn't support
	 * re-initialize, if does so, will cause an exception.
	 * 
	 * @param config
	...
	/**
	 * Get a Logger by a given tag. The tag could be an arbitrary string, and
	 * this tag will be added to the log. In common case, the tag should be the
	 * class name which make use of the Logger.
	 * 
	 * @param tag
	 * @return
	 */
	...
	/**
	 * Get a logger with default tag. The default tag is composited with calling
	...
	public static synchronized Logger getLogger() {
		...
		// }
		// catch (Exception e) {
		// System.out.println("Error occurs during initialize Logger, use
		// System.out instead.");
		// e.printStackTrace(System.out);
	...
	private static Logger createLogger(String tag) throws ConfigurationException, LogInitializationException {
		...
		try {
			...
			Constructor<?> loggerCons = loggerClass
					.getDeclaredConstructor(new Class[] { config.getClass(), String.class });
