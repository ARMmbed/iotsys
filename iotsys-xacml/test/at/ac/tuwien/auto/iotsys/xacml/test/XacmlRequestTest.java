...
public class XacmlRequestTest {
	...
	private static final Logger log = Logger.getLogger(XacmlRequestTest.class.getName());
	...
	public void prepare() {
		...
		request.addSubjectAttribute(XacmlRequest.SUBJECT_ATTRIBUTE_ID, XacmlRequest.XML_SCHEMA_TYPE_STRING,
		...
		request.addResourceAttribute(XacmlRequest.RESOURCE_ATTRIBUTE_ID, XacmlRequest.XML_SCHEMA_TYPE_STRING,
				"45a8f48f70-9d9b-d233-6776-06333cd3c5");
		request.addResourceAttribute(XacmlRequest.RESOURCE_ATTRIBUTE_SERVICENAME, XacmlRequest.XML_SCHEMA_TYPE_STRING,
				"EnergiedatenService");
		...
		request.addActionAttribute(XacmlRequest.ACTION_ATTRIBUTE_ID, XacmlRequest.XML_SCHEMA_TYPE_STRING,
				"getMeterReadings");
	}

	@Test
	public void testAttribute() {
		Attribute av = new Attribute("urn:tuwien:auto:smartwebgrid:subject:role",
	...
	// Assert.assertEquals("Decision objects are not the same",
	// DecisionType.PERMIT, dt);
	// }

	@Test
	public void testEnterpriseXacml() {
		System.setProperty(XACMLParser.CONTEXT_KEY_DEFAULT_SCHEMA_FILE, "xacml-2.0-context.xsd");
		System.setProperty(XACMLParser.POLICY_KEY_DEFAULT_SCHEMA_FILE, "xacml-2.0-policy.xsd");

		try {
			// create request object
			Request xacmlRequest = XACMLParser.parseRequest(new ByteArrayInputStream(request.getRequest().getBytes()));

			// load policy
			String policy = FileHelper.readFile("policies/test-policy.xml");

			AbstractPolicy abstractPolicy = XACMLParser.parsePolicy(new ByteArrayInputStream(policy.getBytes()));
