...
public class CO2Sensor implements EEPParser {
	 ...
	 */
	public CO2Sensor(int rangeMinCO2, int rangeMaxCO2, int rangeMinTemp, int rangeMaxTemp, int rangeMinHumidity,
			int rangeMaxHumidity, int scaleMinCO2, int scaleMaxCO2, int scaleMinTemp, int scaleMaxTemp,
			int scaleMinHumidity, int scaleMaxHumidity) {
	...
	public Map<EnoceanParameterAddress, Value> parsePacket(BasicPacket packet) {
		...
		if (packet instanceof RadioPacket4BS) {
			...
			map.put(new EnoceanParameterAddress(radioPacket4BS.getSenderId(), Parameter.CO2_CONCENTRATION),
					new NumberWithUnit(Unit.PPM, co2));
			BigDecimal temperature = calculationUtil.rangeValue(radioPacket4BS.getDb1(), scaleMinTemp, scaleMaxTemp,
					rangeMinTemp, rangeMaxTemp, 3);
			map.put(new EnoceanParameterAddress(radioPacket4BS.getSenderId(), Parameter.TEMPERATURE),
					new NumberWithUnit(Unit.DEGREE_CELSIUS, temperature));
			BigDecimal humidity = calculationUtil.rangeValue(radioPacket4BS.getDb2(), scaleMinHumidity,
					scaleMaxHumidity, rangeMinHumidity, rangeMaxHumidity, 3);
			map.put(new EnoceanParameterAddress(radioPacket4BS.getSenderId(), Parameter.HUMIDITY),
					new NumberWithUnit(Unit.HUMIDITY, humidity));
		}
		return map;
	}
