...
public class RfidFrame {
	...
	public enum STATES_GET_PACKET {
		STATE_STX(0x01), STATE_STATION_ID(0x02), STATE_LEN(0x03), STATE_DATA(0x04), STATE_BCC(0x05), STATE_ETX(
				0x06), STATE_END(0x7);
	...
	public void readPacket() {
		...
		for (int i = 0; i < packetLength; i++) {
			...
			case STATE_DATA:
				...
				bcc_calc ^= tempData;
				/*
				 * if(data_pos < data_len) { data[data_pos++] = tempData;
				 * bcc_calc ^= tempData; }
				 */
			...
			case STATE_BCC:
				...
				if (bcc_calc != tempData) {
					/*
					 * log.info("wrong bcc!\n"); log.info("bcc recv: " +
					 * Integer.toHexString(tempData)); log.info("calc_bcc: " +
					 * Integer.toHexString(bcc_calc));
					 */
					state = STATES_GET_PACKET.STATE_END;
				} else {
	...
	/*
	 * public RFIDPacket getPacket() { return packet; }
	 * 
	 * public RFIDPacketHeader getPacketHeader() { return header; }
	 */
}
