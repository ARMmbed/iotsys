...
public class RfidDeviceLoaderImpl implements DeviceLoader {
	...
	private final static Logger log = Logger.getLogger(RfidDeviceLoaderImpl.class.getName());
	...
	public ArrayList<Connector> initDevices(ObjectBroker objectBroker) {
		...
		if (connectorsFromDb.size() <= 0) {
			Object rfidConnectors = devicesConfig.getProperty("rfid.connector.name");
		...
		for (int connector = 0; connector < connectorsSize; connector++) {
			HierarchicalConfiguration subConfig = devicesConfig.configurationAt("rfid.connector(" + connector + ")");
			...
			if (enabled) {
				try {
					log.info("Connecting RFID connector to COM Port: " + serialPort);
					...
					log.info(numberOfDevices + " RFID devices found in configuration for connector " + connectorName);
					...
					for (int i = 0; i < numberOfDevices; i++) {
						...
						Device d = new Device(type, ipv6, null, href, name, null, historyCount, historyEnabled,
						...
						if (type != null) {
							...
							try {
								...
								for (int k = 0; k < declaredConstructors.length; k++) {
									if (declaredConstructors[k].getParameterTypes().length == 1) { // constructor
																									// that
																									// takes
																									// the
																									// KNX
										...
										try {
											...
											if (ipv6 != null) {
												objectBroker.addObj(rfidDevice, ipv6);
											} else {
												objectBroker.addObj(rfidDevice);
											}
											...
											if (historyEnabled != null && historyEnabled) {
												if (historyCount != null && historyCount != 0) {
													objectBroker.addHistoryToDatapoints(rfidDevice, historyCount);
												} else {
													objectBroker.addHistoryToDatapoints(rfidDevice);
												}
											}

											if (groupCommEnabled) {
												objectBroker.enableGroupComm(rfidDevice);
											}

											if (refreshEnabled) {
												objectBroker.enableObjectRefresh(rfidDevice);
											}
					...
					// TemperatureSensorImplXBee xBeeTemperatureSensor = new
					// TemperatureSensorImplXBee(
					...
					// // objectBroker.addHistoryToDatapoints(xBeeBrightness,
					// 100);
