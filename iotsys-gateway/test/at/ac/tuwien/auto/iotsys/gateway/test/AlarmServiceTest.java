...
public class AlarmServiceTest extends AbstractGatewayTest {
	...
	public void testAlarmSubject() {
		expect().body(hasXPath("/obj[@is='obix:AlarmSubject']")).body(hasXPath("/obj/int[@name='count']"))
				.body(hasXPath("/obj/op[@name='query' and @in='obix:AlarmFilter' and @out='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/feed[@name='feed' and @in='obix:AlarmFilter' and @of='obix:Alarm']")).when()
				.get("/alarms");
	}
	...
	public void testAlarmGeneration() {
		...
		given().body(String.format(fanspeedPut, 1000, false)).put("/testDevicesAlarm/fanSpeed1");
		assertEquals(initialAlarmCount, getAlarmCount());

		given().body(String.format(fanspeedPut, 3000, true)).put("/testDevicesAlarm/fanSpeed1");
		assertEquals(initialAlarmCount, getAlarmCount());

		given().body(String.format(fanspeedPut, 500, true)).put("/testDevicesAlarm/fanSpeed1");
	...
	public void testRepeatedAlarmGeneration() {
		...
		// off normal
		given().body(String.format(fanspeedPut, 1000, true)).put("/testDevicesAlarm/fanSpeed2");
		assertEquals(initialAlarmCount + 1, getAlarmCount());

		// staying off normal
		given().body(String.format(fanspeedPut, 200, true)).put("/testDevicesAlarm/fanSpeed2");
		assertEquals(initialAlarmCount + 1, getAlarmCount());

		// to normal
		given().body(String.format(fanspeedPut, 3000, true)).put("/testDevicesAlarm/fanSpeed2");
		assertEquals(initialAlarmCount + 1, getAlarmCount());

		// off normal again, new alarm generated
		given().body(String.format(fanspeedPut, 500, true)).put("/testDevicesAlarm/fanSpeed2");
	...
	public void testAlarmObject() {
		...
		// off normal
		given().body(String.format(fanspeedPut, 1000, true)).put("/testDevicesAlarm/fanSpeed3");
		String alarmHref = getLatestAlarm().get("@href");

		expect().body(hasXPath("/obj/ref[@name='source' and @href='/testDevicesAlarm/fanSpeed3']"))
				.body(hasXPath("/obj/abstime[@name='timestamp']")).when().get(alarmHref);
	}
	...
	public void testAckAlarm() {
		...
		source.value().attach(
				new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l)).setAcked(true).setStateful(false));
		...
		expect().body(
				...
				.body(hasXPath("/obj/abstime[@name='ackTimestamp' and @null='true']")).when().get(alarmHref + "/");
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AckAlarmIn'>" + " <str name='ackUser' val='" + ackUser + "'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AckAlarmOut']")).body(hasXPath("/obj/obj[@href='" + alarmHref + "']"))
				.body(hasXPath("/obj/obj/str[@name='ackUser' and not(@null) and @val='" + ackUser + "']"))
				.body(hasXPath("/obj/obj/abstime[@name='ackTimestamp' and not(@null='true')]")).when()
				.post(alarmHref + "/ack");
		long expectedAckTimestamp = System.currentTimeMillis();

		String response = expect()
				.body(hasXPath(
						"/obj/op[@name='ack' and @href='ack' and @in='obix:AckAlarmIn' and @out='obix:AckAlarmOut']"))
				.body(hasXPath("/obj/str[@name='ackUser' and @val='" + ackUser + "']"))
				.body(hasXPath("/obj/abstime[@name='ackTimestamp' and not(@null='true')]")).when().get(alarmHref + "/")
				.asString();

		long ackTimestamp = DatatypeConverter
				.parseDateTime(XmlPath.from(response).getString("obj.abstime.find { it.@name='ackTimestamp' }.@val"))
				.getTimeInMillis();
	...
	public void testAckAlarmWithNullUser() {
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AckAlarmIn'>" + "	<str name='ackUser' null='true'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AckAlarmOut']")).body(hasXPath("/obj/obj[@href='" + alarmHref + "']"))
	...
	public void testStatefulAlarm() {
		BrightnessActuatorImpl source = new BrightnessActuatorImpl();
		source.value().attach(
				new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l)).setAcked(false).setStateful(true));
		int alarms = getAlarmCount();
		...
		expect().body(hasXPath("/obj/abstime[@name='normalTimestamp' and @null='true']")).when().get(alarmHref);

		// to normal
		source.value().set(15l);
		long expectedNormalTimestamp = System.currentTimeMillis();

		String response = expect().body(hasXPath("/obj/abstime[@name='normalTimestamp' and not(@null)]")).when()
				.get(alarmHref).asString();

		long normalTimestamp = DatatypeConverter
				.parseDateTime(XmlPath.from(response).getString("obj.abstime.find { it.@name='normalTimestamp' }.@val"))
				.getTimeInMillis();
		...
		response = expect().body(hasXPath("/obj/abstime[@name='normalTimestamp' and not(@null='true')]")).when()
				.get(alarmHref).asString();
	...
	public void testAlarmSourceStatus() {
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AckAlarmIn'>" + " <str name='ackUser' val='someUser'/>" + "</obj>")
				.post(alarmHref + "/ack");
	...
	public void testCustomAlarmSubject() {
		...
		val.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l).setFlipped(true), true, false)
				.setTarget(source).setAlarmSubject(subject));
	...
	public void testQueryAlarmSubject() throws ParseException {
		...
		expect().body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='4']"))
				.body(hasXPath("/obj/abstime[@name='start']")).body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']")).when().post(subjectHref + "/query");

		// only alarms in time interval [11:00,12:00]
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <abstime name='start' val='2013-09-10T11:00:00+02:00'/>"
						+ "  <abstime name='end'   val='2013-09-10T12:00:00+02:00'/>" + "</obj>")
				.expect().body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='2']")).body(hasXPath("/obj/abstime[@name='start']"))
				...
				.body(hasXPath("/obj/list/obj[@href='" + alarm12.getFullContextPath() + "']")).when()
				.post(subjectHref + "/query");

		// limit to 2 alarms
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <int name='limit' val='2'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='2']"))
				.body(hasXPath("/obj/abstime[@name='start']")).body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj[@href='" + alarm10.getFullContextPath() + "']"))
				.body(hasXPath("/obj/list/obj[@href='" + alarm11.getFullContextPath() + "']")).when()
				.post(subjectHref + "/query");
	}
	...
	public void testQueryAlarmSource() {
		...
		val1.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l).setFlipped(true), false, false)
				.setTarget(source1));

		BrightnessActuatorImpl source2 = new BrightnessActuatorImpl();
		source2.setHref(new Uri(source2href));
		Int val2 = source2.value();
		val2.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l).setFlipped(true), false, false)
				.setTarget(source2));
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <uri name='source' val='" + source1href + "'/>" + "</obj>")
				.expect().body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='1']")).body(hasXPath("/obj/abstime[@name='start']"))
				.body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj/ref[@name='source' and @href='" + source1.getFullContextPath() + "']"))
				.body(not(
						hasXPath("/obj/list/obj/ref[@name='source' and @href='" + source2.getFullContextPath() + "']")))
	...
	public void testQueryUnackedAlarms() {
		...
		expect().body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='3']"))
				.body(hasXPath("/obj/abstime[@name='start']")).body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				...
				.body(hasXPath("/obj/list/obj[@href='" + alarmAcked.getFullContextPath() + "']")).when()
				.post(subjectHref + "/query");

		// unacked = true, return unacked AckAlarms
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <bool name='unacked' val='true'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/abstime[@name='start']")).body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj[@href='" + alarmUnacked.getFullContextPath() + "']"))
				.body(not(hasXPath("/obj/list/obj[@href='" + alarmStateful.getFullContextPath() + "']")))
				.body(not(hasXPath("/obj/list/obj[@href='" + alarmAcked.getFullContextPath() + "']"))).when()
				.post(subjectHref + "/query");

		// unacked = false, return acked AckAlarms
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <bool name='unacked' val='false'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/abstime[@name='start']")).body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
	...
	public void testQueryActiveAlarms() {
		...
		source1.setHref(new Uri(source1href));
		Int val1 = source1.value();
		val1.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l).setFlipped(true), true, false)
				.setTarget(source1).setAlarmSubject(subject));

		BrightnessActuatorImpl source2 = new BrightnessActuatorImpl();
		source2.setHref(new Uri(source2href));
		Int val2 = source2.value();
		val2.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l).setFlipped(true), true, false)
				.setTarget(source2).setAlarmSubject(subject));
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <bool name='active' val='true'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj/ref[@name='source' and @href='" + source1.getFullContextPath() + "']"))
				.body(not(
		...
		// inactive alarms
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <bool name='active' val='false'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(not(
						hasXPath("/obj/list/obj/ref[@name='source' and @href='" + source1.getFullContextPath() + "']")))
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>" + "  <bool name='active' val='true'/>" + "</obj>").expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']")).body(hasXPath("/obj/int[@name='count' and @val='0']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(not(
						hasXPath("/obj/list/obj/ref[@name='source' and @href='" + source1.getFullContextPath() + "']")))
				.body(not(
