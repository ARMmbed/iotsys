...
public class CoapGroupCommImpl extends GroupCommImpl {
	...
	public synchronized Obj joinGroup(Obj in) {
		...
		if (datapoint.getParent() instanceof IoTSySDevice) {
			final String busUri = ((IoTSySDevice) datapoint.getParent()).getBusAddress() + "/" + datapoint.getHref()
					+ "/groupComm/joinGroup";
			...
			try {
				...
				String formatted = String.format("%04X:%04X:%04X:%04X:%04X:%04X:%04X:%04X",
						(((short) inet6.getAddress()[0]) * 256 + inet6.getAddress()[1]) & 0XFFFF,
						(((short) inet6.getAddress()[2]) * 256 + inet6.getAddress()[3]) & 0XFFFF,
						(((short) inet6.getAddress()[4]) * 256 + inet6.getAddress()[5]) & 0XFFFF,
						(((short) inet6.getAddress()[6]) * 256 + inet6.getAddress()[7]) & 0XFFFF,
						(((short) inet6.getAddress()[8]) * 256 + inet6.getAddress()[9]) & 0XFFFF,
						(((short) inet6.getAddress()[10]) * 256 + inet6.getAddress()[11]) & 0XFFFF,
						(((short) inet6.getAddress()[12]) * 256 + inet6.getAddress()[13]) & 0XFFFF,
						(((short) inet6.getAddress()[14]) * 256 + inet6.getAddress()[15]) & 0XFFFF);
	...
	public synchronized Obj leaveGroup(Obj in) {
		...
		if (datapoint.getParent() instanceof IoTSySDevice) {
			final String tempUri = ((IoTSySDevice) datapoint.getParent()).getBusAddress() + "/" + datapoint.getHref()
