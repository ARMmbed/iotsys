...
public class CoAPHelper {
	 ...
	 *            The Obj to encode
	 * @param request
	 *            A Request. Hrefs are encoded relative to the request's path
	...
	/**
	 * Sets the response to the given Request. The response is encoded
	 * appropriately based on the request's accept-header
	 * 
	 * @param obixResponse
	 *            The response to encode
	 * @param request
	 *            The request which is being responded to
	 */
	public void encodeResponse(String obixResponse, Request request) {
		if (request.getFirstAccept() == MediaTypeRegistry.APPLICATION_EXI) {
			try {
				...
				request.respond(CodeRegistry.RESP_CONTENT, exiData, MediaTypeRegistry.APPLICATION_EXI);
			...
			try {
				byte[] exiData = ExiUtil.getInstance().encodeEXI(obixResponse, true);
				request.respond(CodeRegistry.RESP_CONTENT, exiData, MediaTypeRegistry.APPLICATION_EXI);
			...
			try {
				byte[] exiData = BinObixEncoder.toBytes(ObixDecoder.fromString(obixResponse));
				request.respond(CodeRegistry.RESP_CONTENT, exiData, MediaTypeRegistry.APPLICATION_X_OBIX_BINARY);
			...
			try {
				request.respond(CodeRegistry.RESP_CONTENT, JsonUtil.fromXMLtoJSON(obixResponse),
						MediaTypeRegistry.APPLICATION_JSON);
			} catch (JSONException e) {
				e.printStackTrace();
				request.respond(CodeRegistry.RESP_CONTENT, obixResponse.toString(), MediaTypeRegistry.TEXT_XML);
			}
		} else if (request.getUriPath().endsWith(".well-known/core")
				&& !obixServer.containsIPv6(getIPv6Address(request))) {
			obixResponse = obixServer.getCoRELinks();
			request.respond(CodeRegistry.RESP_CONTENT, obixResponse, MediaTypeRegistry.APPLICATION_LINK_FORMAT);
	...
	/**
	 * @param request
	 * @return the ipv6 address of the network interface the request was
	 *         received on
	 */
	public String getIPv6Address(Request request) {
		String localSocket = request.getNetworkInterface().getHostAddress().toString();

		int lastIndex = localSocket.lastIndexOf("%");
		String localSocketSplitted = request.getNetworkInterface().getHostAddress().toString();
