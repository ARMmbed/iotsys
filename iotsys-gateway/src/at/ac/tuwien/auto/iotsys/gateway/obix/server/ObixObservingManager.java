...
public class ObixObservingManager implements ExternalObserver {
	...
	private static final Logger LOG = Logger.getLogger(ObixObservingManager.class.getName());
	...
	public void notifyObservers(String resourcePath) {
		...
		if (resourceObservers != null && resourceObservers.size() > 0) {
			...
			LOG.info(String.format("Notifying observers: %d @ %s", resourceObservers.size(), resourcePath));
			...
			if (check <= 0) {
				...
				LOG.info(String.format("Refreshing observing relationship: %s", resourcePath));
			} else {
			...
			for (ObservingRelationship observer : resourceObservers.values()) {
				...
				try {
					...
					obixResponse = obixResponse.replaceFirst(DEFAULT_OBIX_URL_PROTOCOL, CoAPServer.COAP_URL_PROTOCOL);
	...
	public synchronized void addObserver(GETRequest request) {
		...
		LOG.info(String.format("Established observing relationship: %s @ %s", request.getPeerAddress().toString(),
				resourcePath));
	...
	public void removeObserver(String clientID, LocalResource resource) {
		...
		if (resourceObservers != null && clientObservees != null) {
			if (resourceObservers.remove(clientID) != null && clientObservees.remove(resource.getPath()) != null) {
				LOG.info(String.format("Terminated observing relationship by GET: %s @ %s", clientID,
						resource.getPath()));
				return;
			}
		}
	...
	public void removeObserver(String clientID, int mid) {
		...
		if (toRemove != null) {
			...
			if (resourceObservers == null) {
				LOG.severe(
						String.format("FIXME: ObservingManager has clientObservee, but no resourceObservers (%s @ %s)",
			...
			if (resourceObservers.remove(clientID) != null && clientObservees.remove(toRemove.resourcePath) != null) {
				LOG.info(String.format("Terminated observing relationship by RST: %s @ %s", clientID,
						toRemove.resourcePath));
				return;
			}
		}
	...
	public boolean isObserved(String clientID, String resourcePath) {
		return observersByClient.containsKey(clientID) && observersByClient.get(clientID).containsKey(resourcePath);
	}
	...
	public void updateLastMID(String clientID, String path, int mid) {
		...
		if (clientObservees != null) {
			...
			if (toUpdate != null) {
				...
				LOG.finer(String.format("Updated last MID for observing relationship: %s @ %s", clientID,
						toUpdate.resourcePath));
				return;
			}
		}
