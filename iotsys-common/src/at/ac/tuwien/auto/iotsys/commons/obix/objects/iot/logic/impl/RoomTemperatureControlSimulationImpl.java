	...
	private void doControl() {
		if (enabled.get()) {
			...
			if (windowOpen.get()) {
				...
				ObjectBrokerHelper.getInstance().pullObj(new Uri(LINK_STANDBY_MODE_ACTIVE), false)
						.writeObject(new Bool(true));
				standbyModeActive.set(true);
				ObjectBrokerHelper.getInstance().pullObj(new Uri(LINK_COMFORT_MODE_ACTIVE), false)
						.writeObject(new Bool(false));
				...
				if (roomCurrentTemp.get() < roomTempSetPoint.get() - tolerance.get() + setPointDifference.get()
					...
					ObjectBrokerHelper.getInstance().pullObj(new Uri(AIR_IN_VALVE_LINK), false)
							.writeObject(new Real(100));
					ObjectBrokerHelper.getInstance().pullObj(new Uri(AIR_OUT_VALVE_LINK), false)
							.writeObject(new Real(100));
					ObjectBrokerHelper.getInstance().pullObj(new Uri(HEATING_STATUS_LINK), false)
							.writeObject(new Real(100));
